...
Es ist jedoch anzumerken, dass es für die Zwecke dieser Arbeit keinen Unterschied macht, auf welche Definition wir setzen, da das Spannbaum- und das Wald-Polytop (wie der Name schon sagt) beides Polytope sind und als Abbildung $p$ stets die Projektion in den Raum der $x$-Variablen verwendet wird.
...
x_{\left\{v_1, v_2\right\}} - z_{\left(v_1, v_2\right), u} - z_{\left(v_2, v_1\right), u} & = 0\ \forall \left(\left\{v_1, v_2\right\}, u\right)\in EV_n \label{spt_erweitert_primal2_1} \\
x_{\left\{v_1, v_2\right\}} + \sum_{u\in[n]\backslash\left\{v_1, v_2\right\}} z_{\left(v_1, u\right), v_2} & = 1\ \forall \left(v_1, v_2\right)\in [n]^{\underline{2}} \label{spt_erweitert_primal2_2} \\
...
\begin{itemize}
\item $\frac{1}{2} n \left(n-1\right) \left(n-2\right)$ Gleichungen \eqref{spt_erweitert_primal2_1}
\item $n \left(n-1\right)$ Gleichungen \eqref{spt_erweitert_primal2_2}
...
\right) \\
& \stackrel{\eqref{eq:spt_erweitert_2_2}}{=} & \frac{1}{\left|S\right|} \sum_{\left(u, v\right) \in S^{\underline{2}}} 1 \\
& = & \left|S\right|-1.
\end{eqnarray*}
}
...
Nun wollen wir Beobachtungen notieren, welche wir nutzen werden, um $P^{K_n, spt, symm, 0}$ unter Beibehaltung der Symmetrie so zu vereinfachen, dass das Ergebnis weiterhin eine erweiterte Formulierung des Spannbaum-Polytops bleibt.

\section{$P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}}$}

Die erste Beobachtung ist die, dass im Beweis von Satz \ref{sa:proj_spt_erw} im Schritt
\begin{displaymath}
...
\end{displaymath}
die Gleichung \eqref{eq:spt_erweitert_2_2}
\begin{displaymath}
x_{\left\{v, w\right\}} + \sum_{u\in[n]\backslash\left\{v, w\right\}} z_{\left(v, u\right), w} = 1\ \forall \left(v, w\right)\in [n]^{\underline{2}}
\end{displaymath}
nicht direkt verwendet wurde, sondern eine Gleichung ausreicht, welche man erhält, indem man für vorgegebenes $v$, $w$ die durch $\left(v, w\right)$ und $\left(w, v\right)$ indizierten Gleichungen \eqref{eq:spt_erweitert_2_2} aufaddiert:
\begin{equation}
2 x_{\left\{v, w\right\}} + \sum_{u\in[n]\backslash\left\{v, w\right\}} \left(z_{\left(v, u\right), w} + z_{\left(w, u\right), v} \right) = 2\ \forall \left\{v, w\right\}\in {[n] \choose 2}. \label{eq:erw_symm_eq_neu1}
\end{equation}
...
& \stackrel{\eqref{eq:erw_symm_eq_neu1}}{=} &
\frac{1}{\left|S\right|} \sum_{\left\{u, v\right\} \in {S \choose 2}} 2.
\end{eqnarray*}

Somit gilt:
\begin{Sa}
\label{sa:spt_erw_symmetrisch}
\begin{align}
P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}} & := \left\{
\left(\begin{array}{c}
x \\
z
\end{array}\right)\in\mathbb{R}^{[n]\choose 2}\times \mathbb{R}^{[n]^{\underline{3}}}: \nonumber\right. \\
x_{\left\{v, w\right\}} - z_{\left(v, w\right), u} - z_{\left(w, v\right), u} & = 0\ \forall \left(\left\{v, w\right\}, u\right)\in EV_n \label{eq:spt_erweitert_symm_prim_1} \\
2 x_{\left\{v, w\right\}} + \sum_{u\in[n]\backslash\left\{v, w\right\}} \left(z_{\left(v, u\right), w} + z_{\left(w, u\right), v} \right) & = 2\ \forall \left\{v, w\right\}\in {[n] \choose 2} \label{eq:spt_erweitert_symm_prim_2} \\
z_{\left(v, w\right), u} & \left.\geq 0 \ \forall \left(v, w, u\right) \in \left[n\right]^{\underline{3}}\right\} \label{eq:spt_erweitert_symm_z}
\end{align}
bildet eine symmetrische erweiterte Formulierung des Spannbaum-Polytops.
\end{Sa}

Obgleich das Argument bereits gegeben wurde, um den Beweis von Satz \ref{sa:proj_spt_erw} an Satz \ref{sa:spt_erw_symmetrisch} zu adaptieren, werden wir ihn der Übersicht halber nochmals aufschreiben.

Doch zuvor noch eine Bemerkung über die Anzahl an Gleichungen und Ungleichungen:

\begin{Bem}
\label{bem:spt_erw_symmetrisch}
In der vorliegenden Formulierung wird $P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}}$ durch
\begin{itemize}
\item $\frac{1}{2} n \left(n-1\right) \left(n-2\right)$ Gleichungen \eqref{eq:spt_erweitert_symm_prim_1}
\item $\frac{1}{2} n \left(n-1\right)$ Gleichungen \eqref{eq:spt_erweitert_symm_prim_2}
\item $n \left(n-1\right) \left(n-2\right)$ Nichtnegativitätsbedingungen für $z$-Variablen (Ungleichungen \eqref{eq:spt_erweitert_symm_z})
\end{itemize}
in
\begin{itemize}
\item $\frac{1}{2} n \left(n-1\right)$ $x$-Variablen
...
\begin{bew}
Dass der charakteristische Vektor jedes aufspannenden Baums enthalten ist, folgt unmittelbar aus dem von Satz \ref{sa:proj_spt_erw} unter Beachtung, dass Gleichung \eqref{eq:spt_erweitert_symm_prim_2} die Summe zweier Gleichungen aus \eqref{eq:spt_erweitert_2_2} ist.

Somit ist nur die umgekehrte Inklusion zu zeigen. Die Bedingung $x_{\left\{v, w\right\}} \geq 0 \ \forall \left\{v, w\right\} \in {\left[n\right] \choose 2}$ folgt für $n\geq 3$ sofort aus \eqref{eq:spt_erweitert_symm_prim_1} unter Nutzung von $z_{\left(v, w\right), u} \geq 0 \ \forall \left(v, w, u\right) \in \left[n\right]^{\underline{3}}$ (für $n=2$ leitet man einfach $x_{\left\{1, 2\right\}} = 1$ her).

Sei für die restlichen Ungleichungen $S\subseteq \left[n\right]$ mit $\left|S\right|\geq 2$. Dann haben wir
{
\allowdisplaybreaks
\begin{eqnarray*}
x\left(E(S)\right) & = & \sum_{\left\{u, v\right\} \in {S \choose 2}} x_{\left\{u, v\right\}} \\
& = & \frac{1}{\left|S\right|} \left(\frac{1}{2} \sum_{\left(u, v, w\right) \in S^{\underline{3}}} x_{\left\{u, v\right\}} + \sum_{\left(u, w\right) \in S^{\underline{2}}} x_{\left\{u, w\right\}} \right) \\
& \stackrel{\eqref{eq:spt_erweitert_symm_prim_1}}{=} & \frac{1}{\left|S\right|} \left(\frac{1}{2} \sum_{\left(u, v, w\right) \in S^{\underline{3}}} \left(z_{\left(u, v\right), w} + z_{\left(v, u\right), w}\right) + \sum_{\left(u, w\right) \in S^{\underline{2}}} x_{\left\{u, w\right\}} \right) \\
& = & \frac{1}{2 \left|S\right|} \left(\sum_{\left(u, v, w\right) \in S^{\underline{3}}} \left(z_{\left(u, v\right), w} + z_{\left(w, v\right), u}\right) + 2 \sum_{\left(u, w\right) \in S^{\underline{2}}} x_{\left\{u, w\right\}} \right) \\
& \stackrel{z \geq 0}{\leq} & \frac{1}{2 \left|S\right|} \left(\sum_{\left(u, w\right) \in S^{\underline{2}}} \left(2 x_{\left\{u, w\right\}} + \sum_{v \in \left[n\right] \backslash \left\{u, w\right\}}  \left(z_{\left(u, v\right), w} + z_{\left(w, v\right), u}\right)
\right)\right) \\
& \stackrel{\eqref{eq:spt_erweitert_symm_prim_2}}{=} & \frac{1}{2 \left|S\right|} \left(\sum_{\left(u, w\right) \in S^{\underline{2}}} 2\right) \\
& = & \left|S\right|-1.
\end{eqnarray*}
}

Die Gleichung $x\left({\left[n\right] \choose 2}\right) = n-1$ folgt offensichtlich aus dieser Ungleichungskette unter Beachtung, dass für $S=\left[n\right]$ die Bedingung $z\geq 0$ nicht benutzt wurde.
\end{bew}

\section{Verbesserungen von $P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}}$}

\subsection{Zwischenformulierungen}

\label{sec:vereinf_symm_erw_formulierung}

Die zweite Beobachtung ist die, dass im Beweis von Satz \ref{sa:proj_spt_erw} bzw. Satz \ref{sa:spt_erw_symmetrisch} für den Beweis $z_{\left(v, w\right), u} \geq 0 \ \forall \left(v, w, u\right) \in \left[n\right]^{\underline{3}}$ nicht direkt verwendet wurde, sondern die schwächere Bedingung
\begin{displaymath}
z_{\left(v, w\right), u}+z_{\left(u, w\right), v} \geq 0\ \forall \left(v, w, u\right)\in \left[n\right]^{\underline{3}}\ \left(\textnormal{bzw. }\forall \left(\left\{v, u\right\}, w\right) \in EV_n\right)
\end{displaymath}
ausreicht.

Um daraus eine erweiterte Formulierung zu erhalten, müssen wir lediglich sichern, dass weiterhin $x_{\left\{v, w\right\}}\geq 0 \ \forall \left\{v, w\right\} \in {\left[n\right] \choose 2}$ gilt (dies wurde in genannten Beweisen aus der Nichtnegativität der $z$-Variablen gefolgert). Dies kann man trivial dadurch erreichen, indem man $x_{\left\{v, w\right\}}\geq 0 \ \forall \left\{v, w\right\} \in {\left[n\right] \choose 2}$ einfach zusätzlich fordert.

Diese Tatsachen beachtend folgert man durch nochmalige Betrachtung der Beweise von Satz \ref{sa:proj_spt_erw} bzw. Satz \ref{sa:spt_erw_symmetrisch} sofort folgendes Lemma:

\begin{Le}
\label{le:neue_symmetrische_erw_formulierungen}
\begin{eqnarray*}
P^{K_n, spt, symm, z_{\left(v, w\right), u}+z_{\left(u, w\right), v} \geq 0} & := & \left\{
\left(\begin{array}{c}
x \\
z
\end{array}\right)\in\mathbb{R}^{[n]\choose 2}\times \mathbb{R}^{[n]^{\underline{3}}}: \nonumber\right. \\
x_{\left\{v, w\right\}} - z_{\left(v, w\right), u} - z_{\left(w, v\right), u} & = & 0\ \forall \left(\left\{v, w\right\}, u\right)\in EV_n \\
x_{\left\{v, w\right\}} + \sum_{u\in[n]\backslash\left\{v, w\right\}} z_{\left(v, u\right), w} & = & 1\ \forall \left(v, w\right)\in [n]^{\underline{2}} \\
x_{\left\{v, w\right\}} & \geq & 0 \ \forall \left\{v, w\right\} \in {\left[n\right] \choose 2} \\
z_{\left(v, w\right), u}+z_{\left(u, w\right), v} & \geq & \left. 0\ \forall \left(\left\{v, u\right\}, w\right) \in EV_n \nonumber \right\}
\end{eqnarray*}
und
\begin{eqnarray}
P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}, z_{\left(v, w\right), u}+z_{\left(u, w\right), v} \geq 0} & := & \left\{
\left(\begin{array}{c}
x \\
z
\end{array}\right)\in\mathbb{R}^{[n]\choose 2}\times \mathbb{R}^{[n]^{\underline{3}}}: \nonumber\right. \\
x_{\left\{v, w\right\}} - z_{\left(v, w\right), u} - z_{\left(w, v\right), u} & = & 0\ \forall \left(\left\{v, w\right\}, u\right)\in EV_n \label{eq:xy_aufsummiert} \\
2 x_{\left\{v, w\right\}} + \sum_{u\in[n]\backslash\left\{v, w\right\}} \left(z_{\left(v, u\right), w} + z_{\left(w, u\right), v} \right) & = & 2\ \forall \left\{v, w\right\}\in {[n] \choose 2} \nonumber \\
x_{\left\{v, w\right\}} & \geq & 0 \ \forall \left\{v, w\right\} \in {\left[n\right] \choose 2} \nonumber \\
z_{\left(v, w\right), u}+z_{\left(u, w\right), v} & \geq & 0 \left. \ \forall \left(\left\{v, u\right\}, w\right) \in EV_n \nonumber \right\}
\end{eqnarray}
bilden erweiterte Formulierungen des Spannbaum-Polytops.
\end{Le}

\subsection{$P^{K_n, symm, xy}$}

Nun wollen wir $P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}, z_{\left(v, w\right), u}+z_{\left(u, w\right), v} \geq 0}$ weiter betrachten und hieraus eine erweiterte Formulierung, die wir mit $P^{K_n, symm, xy}$ bezeichnen wollen, herleiten.
...
\begin{Bem}
\label{bem:P_p^symm_xy_vs_P_p^anderes}
Wenn wir das Polytop $P^{K_n, spt, symm, xy}$ betrachten, fallen als Unterschiede gegenüber $P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}, z_{\left(v, w\right), u}+z_{\left(u, w\right), v} \geq 0}$ auf:
\begin{itemize}
\item Um die durch $\left\{v, w, u\right\}$ indizierte Gleichung \eqref{eq:sum_x_minus_sum_y} zu erzeugen, wurden die Gleichungen \eqref{eq:xy_aufsummiert}, welche durch $\left(\left\{u, v\right\}, w\right)$, $\left(\left\{v, w\right\}, u\right)$ und $\left(\left\{w, u\right\}, v\right)$ indiziert werden, aufaddiert.
\item Im daraus resultierenden Gleichungs-/Ungleichungssystem wurden die $z$-Variablen eliminiert, indem $y_{\left\{v, u\right\}, w} := z_{\left(v, w\right), u}+z_{\left(u, w\right), v}$ gesetzt wurde.
\end{itemize}
\end{Bem}

Dass die Ungleichungsbeschreibung von $P^{K_n, spt, symm, xy}$ eine erweiterte Formulierung des \linebreak Spann\-baum-Polytops bildet, ist Aussage des folgenden Satzes, für den wir in den folgenden Unterkapiteln drei unterschiedliche Beweise liefern werden:
...
\subsubsection{Herleitung von Satz \ref{sa:P_p^xy} mittels dualer Programme}

Wenn wir das durch die Ungleichungsbeschreibung von $P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}, z_{\left(v, w\right), u}+z_{\left(u, w\right), v} \geq 0}$ aus Lemma \ref{le:neue_symmetrische_erw_formulierungen} induzierte lineare Programm
\begin{align*}
\max \sum_{e\in {\left[n\right] \choose 2}} c_e x_e & \\
\shortintertext{über }
\left\{ \left(\begin{array}{c}
x \\
z
...
\\
x_{\left\{v, w\right\}} - z_{\left(v, w\right), u} - z_{\left(w, v\right), u} & = 0\ \forall \left(\left\{v, w\right\}, u\right)\in EV_n\ \left(q_{\left\{v, w\right\}, u}\right) \\
2 x_{\left\{v, w\right\}} + \sum_{u\in[n]\backslash\left\{v, w\right\}} \left(z_{\left(v, u\right), w} + z_{\left(w, u\right), v} \right) & = 2\ \forall \left\{v, w\right\}\in {[n] \choose 2} \ \left(r_{\left\{v, w\right\}}\right) \\
x_{\left\{v, w\right\}} & \geq 0 \ \forall \left\{v, w\right\} \in {\left[n\right] \choose 2} \\
z_{\left(v, w\right), u}+z_{\left(u, w\right), v} & \left. \geq 0\ \forall \left(\left\{v, u\right\}, w\right) \in EV_n \ \left(s_{\left\{v, u\right\}, w}\right) \right\}
\end{align*}
dualisieren, erhalten wir folgendes duales Programm:
\begin{align}
\min \sum_{e\in {\left[n\right] \choose 2}} 2 r_e & \nonumber \\
\shortintertext{über }
\left\{ \left(\begin{array}{c}
q \\
r \\
s
\end{array}\right)\in \mathbb{R}^{EV_n} \times \mathbb{R}^{[n]\choose 2} \times \mathbb{R}^{EV_n}: \right. \nonumber
\\
\sum_{u\in[n]\backslash\left\{v, w\right\}} q_{\left\{v, w\right\}, u} + 2 r_{\left\{v, w\right\}} & \geq c_{\left\{v, w\right\}}\ \forall \left\{v, w\right\}\in {[n] \choose 2}\ \left(x_{\left\{v, w\right\}}\right) \nonumber \\
-q_{\left\{v, w\right\}, u} + r_{\left\{v, u\right\}} - s_{\left\{v, u\right\}, w} & = 0\ \forall \left(v, w, u\right)\in [n]^{\underline{3}} \ \left(z_{\left(v, w\right), u}\right) \label{eq:q_symmetrie} \\
s_{\left\{v, w\right\}, u} & \left.\geq 0 \ \forall \left(\left\{v, w\right\}, u\right\} \in EV_n\right\}. \nonumber
\end{align}

Nun betrachten wir dieses duale Programm genauer:
\begin{eqnarray*}
q_{\left\{w, v\right\}, u} & = & q_{\left\{v, w\right\}, u} \\
& \stackrel{\eqref{eq:q_symmetrie}}{=} & r_{\left\{v, u\right\}} - s_{\left\{v, u\right\}, w} \\
& = & r_{\left\{u, v\right\}} - s_{\left\{u, v\right\}, w} \\
& \stackrel{\eqref{eq:q_symmetrie}}{=} & q_{\left\{u, w\right\}, v} \\
& = & q_{\left\{w, u\right\}, v} \\
& \stackrel{\eqref{eq:q_symmetrie}}{=} & r_{\left\{w, v\right\}} - s_{\left\{w, v\right\}, u} \\
& = & r_{\left\{v, w\right\}} - s_{\left\{v, w\right\}, u} \\
& \stackrel{\eqref{eq:q_symmetrie}}{=} & q_{\left\{v, u\right\}, w} \\
& = & q_{\left\{u, v\right\}, w} \\
\end{eqnarray*}
Somit hängt für einen zulässigen Punkt des dualen Programms der Wert der $q$-Variablen nur von den drei gewählten Indizes (ohne Beachtung der Reihenfolge der Indizes) ab -- wir können also $q_{\left\{v, w\right\}, u}$ durch eine neue Variable $q_{\left\{v, w, u\right\}}$ ersetzen, ohne dass sich der Optimalwert des dualen linearen Programms ändert. Damit erhalten wir das folgende lineare Programm:
\begin{align}
\min \sum_{e\in {\left[n\right] \choose 2}} 2 r_e & \nonumber \\
\shortintertext{über }
\left\{ \left(\begin{array}{c}
...
\end{array}\right)\in \mathbb{R}^{EV_n} \times \mathbb{R}^{[n]\choose 2} \times \mathbb{R}^{EV_n}: \right. \nonumber
\\
\sum_{u\in[n]\backslash\left\{v, w\right\}} q_{\left\{v, w, u\right\}} + 2 r_{\left\{v, w\right\}} & \geq c_{\left\{v, w\right\}}\ \forall \left\{v, w\right\}\in {[n] \choose 2}\ \left(x_{\left\{v, w\right\}}\right) \nonumber \\
-q_{\left\{v, w, u\right\}} + r_{\left\{v, u\right\}} - s_{\left\{v, u\right\}, w} & = 0\ \forall \left(v, w, u\right)\in [n]^{\underline{3}} \ \left(z_{\left(v, w\right), u}\right) \label{eq:z_dual_symm} \\
s_{\left\{v, w\right\}, u} & \left.\geq 0 \ \forall \left(\left\{v, w\right\}, u\right\} \in EV_n\right\}. \label{eq:s_geq_0}
\end{align}

Wenn wir nun die $s_{\left\{v, w\right\}, u} \geq 0 \ \forall \left(\left\{v, w\right\}, u\right\} \in EV_n$-Bedingung \eqref{eq:s_geq_0} eliminieren (auch dies ändert nichts am Optimalwert, da Gleichung \eqref{eq:z_dual_symm} symmetrisch bezüglich Vertauschung von $v$ und $u$ ist), so erhalten wir
\begin{align}
\min \sum_{e\in {\left[n\right] \choose 2}} 2 r_e & \nonumber \\
\shortintertext{über }
\left\{ \left(\begin{array}{c}
q \\
r
\end{array}\right)\in \mathbb{R}^{EV_n} \times \mathbb{R}^{[n]\choose 2}: \right. \nonumber
\\
\sum_{u\in[n]\backslash\left\{v, w\right\}} q_{\left\{v, w, u\right\}} + 2 r_{\left\{v, w\right\}} & \geq c_{\left\{v, w\right\}}\ \forall \left\{v, w\right\}\in {[n] \choose 2}\ \left(x_{\left\{v, w\right\}}\right) \nonumber \\
-q_{\left\{v, w, u\right\}} + r_{\left\{v, u\right\}} & \left.\geq 0\ \forall \left(v, w, u\right)\in [n]^{\underline{3}} \ \left(z_{\left(v, w\right), u}\right)\right\}. \label{eq:q_r_symm}
\end{align}

Da offensichtlich auch Ungleichung \eqref{eq:q_r_symm} symmetrisch unter Vertauschung von $v$ und $u$ ist (wodurch jede Gleichung aus \eqref{eq:q_r_symm} doppelt auftaucht), führen wir eine neue Dualvariable (des dualen Problems -- also in Bezug auf die erweiterte Formulierung des Spannbaum-Polytops Primalvariable) $y_{\left\{v, u\right\}, w}$ mit $\left(\left\{v, u\right\}, w\right) \in EV_n$ ein und erhalten:
\begin{align*}
\min \sum_{e\in {\left[n\right] \choose 2}} 2 r_e & \\
\shortintertext{über }
\left\{ \left(\begin{array}{c}
...
\\
\sum_{u\in[n]\backslash\left\{v, w\right\}} q_{\left\{v, w, u\right\}} + 2 r_{\left\{v, w\right\}} & \geq c_{\left\{v, w\right\}}\ \forall \left\{v, w\right\}\in {[n] \choose 2}\ \left(x_{\left\{v, w\right\}}\right) \\
-q_{\left\{v, w, u\right\}} + r_{\left\{v, u\right\}} & \left.\geq 0\ \forall \left(\left\{v, u\right\}, w\right)\in EV_n \ \left(y_{\left\{v, u\right\}, w}\right)\right\}.
\end{align*}

Wenn wir dieses lineare Programm wieder rückdualisieren, so erhalten wir
\begin{align*}
\max \sum_{e\in {\left[n\right] \choose 2}} c_e x_e & \nonumber \\
\shortintertext{über }
\left\{ \left(\begin{array}{c}
...
Das Polytop dieses linearen Programms ist exakt $P^{K_n, spt, symm, xy}$.

Alle der betrachteten linearen Programme haben (unter Nutzung von Eigenschaften der LP-Dualität) den selben Optimalwert. Somit hat das finale lineare Programm für ein vorgegebenes $c \in \mathbb{R}^{\left[n\right] \choose 2}$ den selben Optimalwert wie der eines maximalen aufspannenden Baums mit Kantengewichten $c$ (da das initiale lineare Programm eine erweiterte Formulierung des Spannbaum-Polytops darstellt). Das heißt aber, dass $P^{K_n, spt, symm, xy}$, das Polytop des finalen linearen Programms, ebenso eine erweiterte Formulierung des Spannbaum-Polytops darstellt -- die Aussage von Satz \ref{sa:P_p^xy}.

\subsubsection{Beweis von Satz \ref{sa:P_p^xy} durch Betrachtung vorhergehender Beweise}

Der nächste Beweis, dass $P^{K_n, spt, symm, xy}$ eine erweiterte Formulierung des Spannbaum-Polytops darstellt, ergibt sich aus Betrachtung des (impliziten) Beweises von Lemma \ref{le:neue_symmetrische_erw_formulierungen}, welches aussagt, dass $P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}, z_{\left(v, w\right), u}+z_{\left(u, w\right), v} \geq 0}$ eine erweiterte Formulierung des Spannbaum-Poly\-tops bildet.

Wenn wir uns den Beweis von Satz \ref{sa:spt_erw_symmetrisch} (auf welchem der Beweis von Lemma \ref{le:neue_symmetrische_erw_formulierungen} basiert) nochmals anschauen, so stellen wir fest, dass im Schritt
\begin{eqnarray*}
\frac{1}{\left|S\right|} \left(\frac{1}{2} \sum_{\left(u, v, w\right) \in S^{\underline{3}}} x_{\left\{u, v\right\}} + \sum_{\left(u, w\right) \in S^{\underline{2}}} x_{\left\{u, w\right\}} \right)
& \stackrel{\eqref{eq:spt_erweitert_symm_prim_1}}{=} & \frac{1}{\left|S\right|} \left(\frac{1}{2} \sum_{\left(u, v, w\right) \in S^{\underline{3}}} \left(z_{\left(u, v\right), w} + z_{\left(v, u\right), w}\right) \right. \\
& & \left.+ \sum_{\left(u, w\right) \in S^{\underline{2}}} x_{\left\{u, w\right\}} \right)
\end{eqnarray*}
Gleichung \eqref{eq:spt_erweitert_symm_prim_1}
\begin{eqnarray*}
x_{\left\{v, w\right\}} - z_{\left(v, w\right), u} - z_{\left(w, v\right), u} & = & 0\ \forall \left(\left\{v, w\right\}, u\right)\in EV_n
\end{eqnarray*}
nicht direkt verwendet wird, sondern stets implizit die Summe der durch $\left(\left\{v, w\right\}, u\right)$, \linebreak $\left(\left\{w, u\right\}, v\right)$ und $\left(\left\{u, v\right\}, w\right)$ indizierten Gleichungen dieser Serie verwendet wird.

Um dies einzusehen, summieren wir die entsprechenden Gleichungen auf und erhalten
\begin{eqnarray}
x_{\left\{u, v\right\}} + x_{\left\{v, w\right\}} + x_{\left\{w, u\right\}} 
- z_{\left(u, v\right), w} - z_{\left(v, u\right), w} & & \nonumber \\
- z_{\left(v, w\right), u} - z_{\left(w, v\right), u} 
- z_{\left(w, u\right), v} - z_{\left(u, w\right), v}
& = & 0\ \forall \left\{u, v, w\right\}\in {\left[n\right] \choose 3}. \label{eq:pre_Pxy_1}
\end{eqnarray}

Wir zeigen nun, dass für den oben in Erinnerung gerufenen Beweisschritt die Gleichungsserie \eqref{eq:pre_Pxy_1} anstelle von Gleichungsserie \eqref{eq:spt_erweitert_symm_prim_1} ausreicht:
\begin{eqnarray*}
\frac{1}{\left|S\right|} \left(\frac{1}{2} \sum_{\left(u, v, w\right) \in S^{\underline{3}}} x_{\left\{u, v\right\}}\right. & & \\
\left. + \sum_{\left(u, w\right) \in S^{\underline{2}}} x_{\left\{u, w\right\}} \right) & = & \frac{1}{\left|S\right|} \left(\frac{1}{2} \cdot \frac{1}{3} \sum_{\left(u, v, w\right) \in S^{\underline{3}}} \left(x_{\left\{u, v\right\}} + x_{\left\{v, w\right\}} + x_{\left\{w, u\right\}}\right) %\right. \\
%& & \left. 
+ \sum_{\left(u, w\right) \in S^{\underline{2}}} x_{\left\{u, w\right\}} \right) \\
& \stackrel{\eqref{eq:pre_Pxy_1}}{=} & \frac{1}{\left|S\right|} \left(\frac{1}{2} \cdot \frac{1}{3} \sum_{\left(u, v, w\right) \in S^{\underline{3}}} \left(z_{\left(u, v\right), w} + z_{\left(v, u\right), w} + z_{\left(v, w\right), u} \right.\right. \\
& & \left. + z_{\left(w, v\right), u} + z_{\left(w, u\right), v} + z_{\left(u, w\right), v}\right) %\\
%& & 
\left. + \sum_{\left(u, w\right) \in S^{\underline{2}}} x_{\left\{u, w\right\}} \right) \\
& = & \frac{1}{\left|S\right|} \left(\frac{1}{2} \sum_{\left(u, v, w\right) \in S^{\underline{3}}} \left(z_{\left(u, v\right), w} + z_{\left(v, u\right), w}\right) + \sum_{\left(u, w\right) \in S^{\underline{2}}} x_{\left\{u, w\right\}} \right).
\end{eqnarray*}
...
- z_{\left(v, w\right), u} - z_{\left(w, v\right), u} 
- z_{\left(w, u\right), v} - z_{\left(u, w\right), v}
& = & 0\ \forall \left\{u, v, w\right\}\in {\left[n\right] \choose 3} \\
2 x_{\left\{v, w\right\}} + \sum_{u\in[n]\backslash\left\{v, w\right\}} \left(z_{\left(v, u\right), w} + z_{\left(w, u\right), v} \right) & = & 2\ \forall \left\{v, w\right\}\in {[n] \choose 2} \\
x & \geq & 0 \\
z_{\left(v, w\right), u}+z_{\left(u, w\right), v} & \geq & 0 \left. \ \forall \left(\left\{v, u\right\}, w\right) \in EV_n \nonumber \right\}
\end{eqnarray*}
...
Nun wollen wir aus Lemma \ref{le:pre_Pxy} folgern, dass $P^{K_n, spt, symm, xy}$ eine erweiterte Formulierung des Spannbaum-Polytops bildet.
...
Somit folgt Satz \ref{sa:P_p^xy} aus Lemma \ref{le:pre_Pxy} und Lemma \ref{le:variablen_eliminieren}, wobei in Lemma \ref{le:variablen_eliminieren} gesetzt werde:
\begin{itemize}
...
\subsubsection{Ein direkter Beweis von Satz \ref{sa:P_p^xy}}
...
\begin{bew}
Dass sich jeder aufspannende Baum in $P^{K_n, spt, symm, xy}$ einbetten lässt, folgt aus Bemerkung \ref{bem:P_p^symm_xy_vs_P_p^anderes}, sowie den Beweisen von Satz \ref{sa:proj_spt_erw} und Satz \ref{sa:spt_erw_symmetrisch}.
...
\begin{eqnarray*}
EV_n^{\not>} & := & \left\{\left(\left\{v, w\right\}, u\right) \in EV_n: \min\left(v, w\right) < u\right\}, \\
...
S^{\underline{3}^{2<3<1}} & := & \left\{\left(v, w, u\right) \in S^{\underline{3}}: w<u<v\right\} \\
S^{\underline{3}^{1,2\not>3}} & := & \left\{\left(v, w, u\right) \in S^{\underline{3}}: \min\left(v, w\right) < u\right\} \\
& = & S^{\underline{3}^{1<3}} \cupdot S^{\underline{3}^{2<3<1}} \\
S^{\underline{3}^{1,2<3}} & := & \left\{\left(v, w, u\right) \in S^{\underline{3}}: \max\left(v, w\right) < u\right\}.
\end{eqnarray*}
\end{Def}

\section{Elimination von Gleichungen ($P^{K_n, spt, asymm, 1}$)}

\label{sec:asymm_bew}

In Computerexperimenten haben wir zu dem linearen Optimierungsproblem, für vorgegebene Kantengewichte unter Nutzung von $P^{K_n, spt, symm, 0}$ einen optimalen aufspannenden Baum zu berechnen, Lösungen für das dazu duale Optimierungsproblem berechnen lassen.

Hierbei fiel uns auf, dass überraschend viele Dualvariablen auf 0 gesetzt wurden. Dies lässt vermuten, dass zahlreiche Restriktionen in $P^{K_n, spt, symm, 0}$ überflüssig sind (in dem Sinne, dass man sie einfach weglassen kann und nicht wie in Kapitel \ref{chap:neu_symm}, wo wir durch Aufsummieren der Gleichungen bzw. Ungleichungen schwächere, aber für die Eigenschaft einer erweiterten Formulierung ausreichende, Restriktionen hergeleitet haben).

In der Tat werden wir nun zeigen, dass ein Polytop, das wir mit $P^{K_n, spt, asymm, 1}$ bezeichnen und die durch das Weglassen von Gleichungen aus $P^{K_n, spt, symm, 0}$ entstanden ist (einzige Ausnahme: wir fügen die zusätzliche Ungleichung $x_{\left\{n-1, n\right\}} \geq 0$ ein -- in Lemma \ref{le:xn-1, ngeq0} werden wir zeigen, dass dies für $n \geq 4$ tatsächlich erforderlich ist), tatsächlich eine Erweiterung (und somit seine Gleichungs-/Ungleichungsbeschreibung eine erweiterte Formulierung) des Spannbaum-Polytops bildet.

Um dies zu zeigen, werden wir die Gleichungen und Ungleichungen auf eine alternative Art aufsummieren.

\begin{Sa}
\label{sa:spt_erw_asymmetrisch}
\begin{align}
P^{K_n, spt, asymm, 1} & := \left\{
\left(\begin{array}{c}
x \\
z
\end{array}\right)\in \mathbb{R}^{[n]\choose 2}\times \mathbb{R}^{\left[n\right]^{\underline{3}^{1,2\not>3}}}: \nonumber\right. \\
x_{\left\{v, w\right\}} - z_{\left(v, w\right), u} - z_{\left(w, v\right), u} & = 0\ \forall \left(\left\{v, w\right\}, u\right)\in EV_n^{\not>} \label{eq:spt_erweitert_asymm_prim_1} \\
x_{\left\{v, w\right\}} + \sum_{u\in[n]\backslash\left\{v, w\right\}} z_{\left(v, u\right), w} & = 1\ \forall \left(v, w\right)\in {[n]^{\underline{2}}}^< \label{eq:spt_erweitert_asymm_prim_2} \\
x_{\left\{n-1, n\right\}} & \geq 0 \label{eq:spt_erweitert_asymm_x_n-1n} \\
z_{\left(v, w\right), u} & \left. \geq 0 \ \forall \left(v, w, u\right) \in \left[n\right]^{\underline{3}^{1,2\not>3}} \right\} \label{eq:spt_erweitert_asymm_z}
\end{align}
bildet eine asymmetrische erweiterte Formulierung des Spannbaum-Polytops.
\end{Sa}

Bevor wir diese Aussage beweisen, bringen wir noch eine Bemerkung über die Anzahl an Gleichungen und Ungleichungen:
\begin{Bem}
\label{bem:gleichungen_asymm}
In der vorliegenden Formulierung wird $P^{K_n, spt, asymm, 1}$ durch
\begin{itemize}
\item $\frac{1}{3} n \left(n-1\right) \left(n-2\right)$ Gleichungen \eqref{eq:spt_erweitert_asymm_prim_1}
\item $\frac{1}{2} n \left(n-1\right)$ Gleichungen \eqref{eq:spt_erweitert_asymm_prim_2}
\item  $1$ Nichtnegativitätsbedingung für $x_{\left\{n-1, n\right\}}$ \eqref{eq:spt_erweitert_asymm_x_n-1n}
\item $\frac{2}{3} n \left(n-1\right) \left(n-2\right)$ Nichtnegativitätsbedingungen für $z$-Komponenten \eqref{eq:spt_erweitert_asymm_z}
\end{itemize}
in 
\begin{itemize}
\item $\frac{1}{2} n \left(n-1\right)$ $x$-Variablen
\item $\frac{2}{3} n \left(n-1\right) \left(n-2\right)$ $z$-Variablen
\end{itemize}
beschrieben.
\end{Bem}
\begin{bew}
Um die Anzahl Gleichungen \eqref{eq:spt_erweitert_asymm_prim_1} zu berechnen, schauen wir uns an, wie viele Paare $\left(k, l\right) \in \left[n\right]^{\underline{2}}$ mit $k<l$ es für ein vorgegebenes $k$ gibt.

Die Antwort darauf ist offenbar $n-k$.

Nun sei für ein vorgegebenes $k$ ein solches Paar $\left(k, l\right)\cong \left\{k, l\right\}$ vorgegeben.

Dann gibt es $n-k-1$ Möglichkeiten für ein $m$, so dass nicht $m\leq k\vee m=l$ gilt.

Dies macht in der Summe
\begin{displaymath}
\sum_{k=1}^{n-1} \left(n-k\right) \left(n-k-1\right) = \frac{n \left(n-1\right) \left(n-2\right)}{3}
\end{displaymath}
Gleichungen vom Typ \eqref{eq:spt_erweitert_asymm_prim_1}.

Die Anzahl an Gleichungen von Typ \eqref{eq:spt_erweitert_asymm_prim_2} ist offensichtlich.

Für die Anzahl an Nichtnegativitätsbedingungen bzw. $z$-Variablen beachte man, dass jedem Element aus der Menge $EV_n^{\not>}$ (deren Kardinalität wir soeben bestimmt haben) zwei Elemente aus $\left[n\right]^{\underline{3}^{1,2\not>3}}$ gegenüberstehen.

Somit erhalten wir $\frac{2}{3} n \left(n-1\right) \left(n-2\right)$ $z$-Variablen.
\end{bew}

\begin{bew} (Satz \ref{sa:spt_erw_asymmetrisch})
Die Gleichungen \eqref{eq:spt_erweitert_asymm_prim_1} und \eqref{eq:spt_erweitert_asymm_prim_2} bilden eine Teilmenge der Gleichungen \eqref{eq:spt_erweitert_2_1} und \eqref{eq:spt_erweitert_2_2} -- somit ist die eine Inklusion offensichtlich.

Für die andere Richtung zeigen wir zuerst $x_{\left\{v, w\right\}}\geq 0 \ \forall \left\{v, w\right\} \in {\left[n\right] \choose 2}$. 

Für $\left(v, w\right)\in [n]^{\underline{2}^<}\backslash \left\{\left(n-1, n\right)\right\}$ folgt aus der durch $\left(v, w\right)$ indizierten Gleichung \eqref{eq:spt_erweitert_asymm_prim_1} (unter Nutzung von $z_{\left(v, w\right), u} \geq 0 \ \forall \left(v, w, u\right) \in \left[n\right]^{\underline{3}^{1,2\not>3}}$): $x_{\left\{v, w\right\}} \geq 0$. $x_{\left\{n-1, n\right\}} \geq 0$ gilt wegen \eqref{eq:spt_erweitert_asymm_x_n-1n}.
...
& \stackrel{\eqref{eq:spt_erweitert_asymm_prim_1}}{=} & \sum_{\left\{v, w\right\} \in {S \backslash \left\{s_{max}\right\} \choose 2}} \left(z_{\left(v, w\right), s_{max}} + z_{\left(w, v\right), s_{max}}\right) + \sum_{v \in S \backslash \left\{s_{max}\right\}} x_{\left\{v, s_{max}\right\}} \\
...
& \stackrel{\eqref{eq:spt_erweitert_asymm_prim_2}}{=} & \sum_{v \in S \backslash \left\{s_{max}\right\}} 1 \\
& = & \left|S\right| - 1.
\end{eqnarray*}
}

Die Bedingung $x\left({\left[n\right] \choose 2}\right) = n-1$ folgt offensichtlich aus dieser Ungleichungskette unter Beachtung, dass für $S=\left[n\right]$ die Bedingung $z_{\left(v, w\right), u} \geq 0 \ \forall \left(v, w, u\right) \in \left[n\right]^{\underline{3}^{1,2\not>3}}$ nicht benutzt wurde.
\end{bew}

\section{Elimination von Nichtnegativitätsbedingungen ($P^{K_n, spt, asymm, 2}$)}

Als erstes beobachtet man, dass im Beweis von Satz \ref{sa:spt_erw_asymmetrisch} der Schlupf in der Ungleichung
\begin{displaymath}
x\left(E\left(S\right)\right) \leq \left|S\right|-1
\end{displaymath}
gleich
\begin{displaymath}
\sum_{
\substack{
v\in S \backslash \left\{s_{max}\left(S\right)\right\}, \\
w\in \left[n\right]\backslash S
}
} z_{\left(v, w\right), s_{max}}
\end{displaymath}
ist.

Daher ist es als erster Schritt sinnvoll, die Menge
\begin{displaymath}
\bigcup_{
\substack{
S \subseteq \left[n\right], \\
v\in S \backslash \left\{s_{max}\left(S\right)\right\}, \\
w\in \left[n\right]\backslash S
}
}
 \left\{\left(v, w, s_{max}\left(S\right)\right)\right\}
\end{displaymath}
zu charakterisieren.
...
\left\{\left(v, w, \max\left(S\right)\right)\right\}
= \left[n\right]^{\underline{3}^{1<3}}
\end{displaymath}
\end{Le}
\begin{bew}
"`$\subseteq$"': klar, weil wegen $v<s_{max}\left(S\right)$ sicher die erste Komponente kleiner der dritten ist. \\

"`$\supseteq$"': Sei $\left(v_1, v_2, v_3\right) \in \left[n\right]^{\underline{3}^{1<3}}$. Setze $\widetilde{S}:=\left\{v_1, v_3\right)$. Dann ist sicherlich
\begin{displaymath}
\left(v_1, v_2, v_3\right) \in
\bigcup_{
\substack{
v\in \widetilde{S} \backslash \left\{\max\left(\widetilde{S}\right)\right\}, \\
w\in \left[n\right]\backslash \widetilde{S}
}
}
\left\{\left(v, w, \max\left(\widetilde{S}\right)\right)\right\}, 
\end{displaymath}
also auch
\begin{displaymath}
\left(v_1, v_2, v_3\right) \in \bigcup_{
\substack{
S \subseteq \left[n\right], \\
v\in S \backslash \left\{\max\left(S\right)\right\}, \\
w\in \left[n\right]\backslash S
}
}
\left\{\left(v, w, s_{max}\left(S\right)\right)\right\}.
...
Dieses Lemma können wir ausnutzen, um eine neue asymmetrische erweiterte Formulierung des Spannbaum-Polytops zu erzeugen:

\begin{Sa}
\label{sa:asymmetrisch_erweitert2}
\begin{align}
P^{K_n, spt, asymm, 2} := \left\{
\left(\begin{array}{c}
x \\
z
\end{array}\right)\in \mathbb{R}^{[n]\choose 2}\times \mathbb{R}^{\left[n\right]^{\underline{3}^{1,2\not>3}}}: \nonumber\right. \\
x_{\left\{v, w\right\}} - z_{\left(v, w\right), u} - z_{\left(w, v\right), u} & = 0\ \forall \left(\left\{v, w\right\}, u\right)\in EV_n^{\not>} \label{eq:spt_erweitert_asymm2_prim_1} \\
x_{\left\{v, w\right\}} + \sum_{u\in[n]\backslash\left\{v, w\right\}} z_{\left(v, u\right), w} & = 1\ \forall \left(v, w\right)\in {[n]^{\underline{2}}}^< \label{eq:spt_erweitert_asymm2_prim_2} \\
x_{\left\{v, n\right\}} & \geq 0 \ \forall v \in \left[n-1\right] \label{eq:spt_erweitert_asymm2_x_n} \\
z_{\left(v, w\right), u} & \left. \geq 0 \ \forall \left(v, w, u\right)\in \left[n\right]^{\underline{3}^{1<3}} \label{eq:spt_erweitert_asymm2_z} \right\}
\end{align}
bildet eine asymmetrische erweiterte Formulierung des Spannbaum-Polytops.
\end{Sa}
\begin{bew}
Da die Gleichungen und Ungleichungen eine Teilmenge der für die erweiterte Formulierung aus Satz \ref{sa:spt_erw_asymmetrisch} gültigen Gleichungen und Ungleichungen darstellen, lässt sich sicherlich jeder Baum einbetten.

Nun zur Rückrichtung: die Ungleichungen bzw. Gleichung
\begin{eqnarray*}
x\left(E\left(S\right)\right) & \leq & \left|S\right|-1 \\
x\left(E\left(\left[n\right]\right)\right) & = & n-1
\end{eqnarray*}
folgen aus der vorhergehenden Betrachtung.

Damit ist nur $x_{\left\{v, w\right\}}\geq 0$ sicherzustellen. Falls $n\in \left\{v, w\right\}$ ist, folgt dies sofort aus \eqref{eq:spt_erweitert_asymm2_x_n}.

Sei daher $n\notin \left\{v, w\right\}$.

Dann gilt
\begin{eqnarray*}
x_{\left\{v, w\right\}} & \stackrel{\eqref{eq:spt_erweitert_asymm2_prim_1}}{=} & z_{\left(v, w\right), n} + z_{\left(w, v\right), n} \\
& \stackrel{\eqref{eq:spt_erweitert_asymm2_z}}{\geq} & 0.
\end{eqnarray*}
\end{bew}

\begin{Bem}
$P^{K_n, spt, asymm, 2}$ wird in Satz \ref{sa:asymmetrisch_erweitert2} durch
\begin{itemize}
\item $\frac{1}{3} n \left(n-1\right) \left(n-2\right)$ Gleichungen \eqref{eq:spt_erweitert_asymm2_prim_1}
\item $\frac{1}{2} n \left(n-1\right)$ Gleichungen \eqref{eq:spt_erweitert_asymm2_prim_2}
\item $n-1$ Nichtnegativitätsbedingungen für $x$-Komponenten \eqref{eq:spt_erweitert_asymm2_x_n}
\item $\frac{1}{2} n \left(n-1\right) \left(n-2\right)$ Nichtnegativitätsbedingungen für $z$-Komponenten \eqref{eq:spt_erweitert_asymm2_z}
\end{itemize}
in 
\begin{itemize}
\item $\frac{1}{2} n \left(n-1\right)$ $x$-Variablen
...
\section{Elimination von Variablen ($P^{K_n, spt, asymm, 3}$ und $P^{K_n, spt, asymm, 4}$)}

Als nächstes fällt auf, dass in der erweiterten Formulierung aus Satz \ref{sa:asymmetrisch_erweitert2} jede der Variablen $z_{\left(v, w\right), u}$ mit $v>u$ in genau einer Gleichung (nämlich vom Typ \eqref{eq:spt_erweitert_asymm2_prim_1}) auftritt.

Da eine solche Variable nicht vorzeichenbeschränkt ist, kann sie jeden beliebigen Wert annehmen -- die entsprechende Gleichung bringt also keine Einschränkung, da wir die entsprechende $z_{\left(v, w\right), u}$-Variable mit $v>u$ immer derart setzen können, dass die Gleichung stets erfüllt ist.

Präziser: setze für $v>u$
\begin{displaymath}
z_{\left(v, w\right), u} := x_{\left\{v, w\right\}} - z_{\left(w, v\right), u}.
\end{displaymath}

Wenn wir die entsprechenden Gleichungen entfernen, erhalten wir
\begin{align*}
\left\{
\left(\begin{array}{c}
x \\
...
z_{\left(v, w\right), u} & \left. \geq 0 \ \forall \left(v, w, u\right)\in \left[n\right]^{\underline{3}^{1<3}} \right\}.
\end{align*}

In den resultierenden Gleichungen treten keine Variablen $z_{\left(v, w\right), u}$ mit $v>u$ auf -- diese können wir demnach auch entfernen und erhalten
\begin{align*}
\left\{
\left(\begin{array}{c}
x \\
...
Dass dies eine erweiterte Formulierung des Spannbaum-Polytops bildet, ist Inhalt des nächsten Satzes.
...
\end{Sa}
\begin{bew}
Man überprüft leicht, dass der vorgestellte Beweis von Satz \ref{sa:asymmetrisch_erweitert2}, dass sich jeder Baum einbetten lässt und die Gleichungen/Ungleichungen
\begin{eqnarray*}
x\left(E\left(S\right)\right) \leq \left|S\right|-1 \ \forall S \subset \left[n\right], \left|S\right| & \geq & 2, \\
x\left(E\left(\left[n\right]\right)\right) & = & n-1
\shortintertext{und}
x_{\left\{v, w\right\}} & \geq & 0 \ \forall \left\{v, w\right\} \in {\left[n\right] \choose 2}
\end{eqnarray*}
erfüllt sind, weiterhin funktioniert.
\end{bew}
...
\end{Bem}
\begin{bew}
Lediglich die Anzahl von Gleichungen vom Typ \eqref{eq:spt_erweitert_asymm3_prim_1} ist nicht unmittelbar ersichtlich oder aus vorhergehenden Resultaten bekannt.
...
\end{Sa}
\begin{bew}
Obgleich der Beweis analog zu dem Beweis von Satz \ref{sa:spt_erw_asymmetrisch} erfolgt, wollen wir ihn explizit aufschreiben.
...
Als nächstes entwickeln wir eine zu $P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}}$ analoge erweiterte Formulierung des Wald-Polytops, die wir mit $P^{K_n, for, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}}$ bezeichnen wollen.

Auch hier zeigt man absolut analog zum Beweis von Satz \ref{sa:baum_erweitert_1}, nur diesmal unter Nutzung des Beweises von Satz \ref{sa:spt_erw_symmetrisch}:
\begin{Sa}
\begin{align*}
P^{K_n, for, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}} & := \left\{
\left(\begin{array}{c}
x \\
z
\end{array}\right)\in\mathbb{R}^{[n]\choose 2}\times \mathbb{R}^{[n]^{\underline{3}}}: \nonumber\right. \\
x_{\left\{v_1, v_2\right\}} - z_{\left(v_1, v_2\right), u} - z_{\left(v_2, v_1\right), u} & = 0\ \forall \left(\left\{v_1, v_2\right\}, u\right)\in EV_n \\
2 x_{\left\{v_1, v_2\right\}} + \sum_{u\in[n]\backslash\left\{v_1, v_2\right\}} \left(z_{\left(v_1, u\right), v_2} + z_{\left(v_2, u\right), v_1} \right) & \leq 2\ \forall \left\{v_1, v_2\right\}\in {[n] \choose 2} \\
z_{\left(v_1, v_2\right), v_3} & \geq 0 \nonumber \left. \ \forall \left(v_1, v_2, v_3\right) \in \left[n\right]^{\underline{3}}\right\}
\end{align*}
bildet eine erweiterte Formulierung des Wald-Polytops $P^{K_n, for}$.
\end{Sa}
...
Aus Platzgründen wollen wir in diesem Unterkapitel lediglich die zu den erweiterten Formulierungen $P^{K_n, spt, asymm, 1}$ und $P^{K_n, spt, asymm, 4}$ des Spannbaum-Polytops $P^{K_n, spt}$ korrespondierenden erweiterten Formulierungen $P^{K_n, for, asymm, 1}$ bzw. $P^{K_n, for, asymm, 4}$ des Wald-Polytops $P^{K_n, for}$ angeben. Es ist jedoch einfach möglich, auch aus $P^{K_n, spt, asymm, 2}$ und $P^{K_n, spt, asymm, 3}$ erweiterte Formulierungen von  $P^{K_n, for}$ mittels einer analogen Konstruktion zu erhalten. 

Analog zum Beweis von Satz \ref{sa:baum_erweitert_1} (selbe Einbettung von Wäldern) unter Nutzung des Beweises von Satz \ref{sa:spt_erw_asymmetrisch} (wobei man wie im Beweis von diesem Satz die Gleichungen umnummeriert und ggf. Gleichheits-Zeichen durch $\leq$-Zeichen ersetzt) beweist man:

\begin{Sa}
\begin{align}
P^{K_n, for, asymm, 1} & := \left\{
\left(\begin{array}{c}
x \\
z
\end{array}\right)\in \mathbb{R}^{[n]\choose 2}\times \mathbb{R}^{\left[n\right]^{\underline{3}^{1,2\not>3}}}: \nonumber\right. \\
x_{\left\{v, w\right\}} - z_{\left(v, w\right), u} - z_{\left(w, v\right), u} & = 0\ \forall \left(\left\{v, w\right\}, u\right)\in EV_n^{\not>} \label{eq:forest_erweitert_asymm_prim_1} \\
x_{\left\{v, w\right\}} + \sum_{u\in[n]\backslash\left\{v, w\right\}} z_{\left(v, u\right), w} & \leq 1\ \forall \left(v, w\right)\in {\left[n\right]^{\underline{2}}}^< \label{eq:forest_erweitert_asymm_prim_2} \\
x_{\left\{n-1, n\right\}} & \geq 0 \label{eq:forest_erweitert_asymm_x} \\
z_{\left(v, w\right), u} & \left. \geq 0 \ \forall \left(v, w, u\right) \in \left[n\right]^{\underline{3}^{1,2\not>3}} \right\} \label{eq:forest_erweitert_asymm_z}
\end{align}
bildet eine asymmetrische erweiterte Formulierung des Wald-Polytops $P^{K_n, for}$.
\end{Sa}

\begin{Bem}
In der vorliegenden Formulierung wird $P^{K_n, for, asymm, 1}$ durch
\begin{itemize}
\item $\frac{1}{3} n \left(n-1\right) \left(n-2\right)$ Gleichungen \eqref{eq:forest_erweitert_asymm_prim_1}
\item $\frac{1}{2} n \left(n-1\right)$ Ungleichungen \eqref{eq:forest_erweitert_asymm_prim_2}
\item $1$ Nichtnegativitätsbedingung für $x_{\left\{n-1, n\right\}}$ \eqref{eq:forest_erweitert_asymm_x}
\item $\frac{2}{3} n \left(n-1\right) \left(n-2\right)$ Nichtnegativitätsbedingungen für $z$-Komponenten \eqref{eq:forest_erweitert_asymm_z}
\end{itemize}
in 
\begin{itemize}
\item $\frac{1}{2} n \left(n-1\right)$ $x$-Variablen
...
Wieder beweist man analog zum Beweis von Satz \ref{sa:baum_erweitert_1}, diesmal unter Nutzung des Beweises von Satz \ref{sa:asymmetrisch_erweitert4}:
...
\chapter{Der Zusammenhang zwischen den erweiterten Formulierungen}

\section{Der Zusammenhang zwischen $P^{K_n, spt, symm, 0}$ und $P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}}$}

\begin{Le}
Für  $n\geq 3$ ist
\begin{displaymath}
P^{K_n, spt, symm, 0} \subset P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}}
\end{displaymath}
(für $n\leq 2$ gilt Gleichheit).
\end{Le}
\begin{bew}
Dass für $n\leq 2$ Gleichheit gilt, ist trivial nachzurechnen.

Es ist offensichtlich, dass $P^{K_n, spt, symm, 0} \subseteq P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}}$ erfüllt ist. Daher ist nur zu zeigen, dass die Inklusion für $n\geq 3$ echt ist.

Dies folgt direkt aus Satz \ref{sa:dimension_formulierungen} wegen
\begin{eqnarray*}
\dim P^{K_n, spt, symm, 0} & = & \frac{\left(n-1\right)^2 \left(n-2\right)}{2} \\
& < & \frac{n \left(n-1\right)\left(n-2\right)}{2} \\
& = & \dim P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}}.
\end{eqnarray*}

Wir wollen jedoch direkt einen Punkt angeben, welcher in $P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}}$, nicht jedoch in $P^{K_n, spt, symm, 0}$ liegt:

Setze
\begin{eqnarray*}
x_e & = & \frac{2}{n}\ \forall e\in {\left[n\right] \choose 2} \\
z_{\left(v, w\right), u} & = & \begin{cases}
\frac{2}{n} & \textnormal{für }\left(v, w, u\right)\in \left\{\left(1, 2, 3\right), \left(2, 3, 1\right), \left(3, 1, 2\right)\right\} \\
0 & \textnormal{für }\left(v, w, u\right)\in \left\{\left(3, 2, 1\right), \left(1, 3, 2\right), \left(2, 1, 3\right)\right\} \\
\frac{1}{n} & \textnormal{sonst}
\end{cases}
\end{eqnarray*}
...
\begin{eqnarray*}
x_{\left\{1, 2\right\}} + \sum_{u\in \left[n\right]\backslash \left\{1, 2\right\}} z_{\left(1, u\right), 2} & = & \frac{2}{n} + \underbrace{z_{\left(1, 3\right), 2}}_{= 0}+\left(n-3\right)\frac{1}{n} \\
& = & \frac{n-1}{n} \\
& \neq & 1,
\end{eqnarray*}
also ist $\left(
...
z
\end{array}
\right) \notin P^{K_n, spt, symm, 0}$.

Auf der anderen Seite haben wir:
\begin{eqnarray*}
x_{\left\{v, w\right\}} - z_{\left(v, w\right), u} - z_{\left(w, v\right), u} & = & 0\ \forall \left(\left\{v, w\right\}, u\right)\in EV_n \\
2 x_{\left\{v, w\right\}} + \sum_{u\in[n]\backslash\left\{v, w\right\}} \left(z_{\left(v, u\right), w} + z_{\left(w, u\right), v} \right) & = & 2\ \forall \left\{v, w\right\}\in {[n] \choose 2} \\
z_{\left(v, w\right), u} & \geq & 0 \ \forall \left(v, w, u\right) \in \left[n\right]^{\underline{3}},
\end{eqnarray*}
weswegen $\left(
\begin{array}{c}
x \\
z
\end{array}
\right) \in P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}}$ ist.
\end{bew}

\section{Der Zusammenhang zwischen $P^{K_n, spt, symm, 0}$ und $P^{K_n, spt, asymm, 1}$}

Angesichts der Gleichungs-/Ungleichungsstruktur von $P^{K_n, spt, asymm, 1}$ könnte man vermuten, dass gilt:
\begin{equation}
\proj_{\mathbb{R}^{\left[n\right] \choose 2} \times \mathbb{R}^{\left[n\right]^{\underline{3}^{1,2\not>3}}}} P^{K_n, spt, symm, 0} = P^{K_n, spt, asymm, 1}. \label{eq:lift_polytop}
\end{equation}

In diesem Abschnitt werden wir zeigen, dass dies \emph{nicht} der Fall ist.

Aber zuerst ein Satz, welcher aufzeigt, dass zumindest ein kleiner Teil dieser Vermutung (dass nämlich eine spezielle für $P^{K_n, spt, symm, 0}$ gültige Gleichung auch für $P^{K_n, spt, asymm, 1}$ erfüllt ist) gilt und Bedingungen liefert, die notwendig sind, damit wir einen Punkt aus $P^{K_n, spt, asymm, 1}$ nach $P^{K_n, spt, symm, 0}$ "`hochliften"' können (d. h. zu einem Punkt $\left(\begin{array}{c}
x \\
z
\end{array}\right) \in P^{K_n, spt, asymm, 1}$ einen Punkt $\left(\begin{array}{c}
x \\
\widetilde{z}
\end{array}\right) \in P^{K_n, spt, symm, 0}$ finden, welcher  $\widetilde{z}_{\left(v, w\right), u} = z_{\left(v, w\right), u}$ für $\left(v, w, u\right)\in \left[n\right]^{\underline{3}^{1,2\not>3}}$ erfüllt).

Letzteres werden wir dann nutzen, um zu zeigen, dass \eqref{eq:lift_polytop} für $n \geq 4$ \emph{nicht} gilt.

\begin{Sa}
\label{sa:P_p^asymm_vs_P_p^2}
Sei $\left(\begin{array}{c}
x \\
z
\end{array}\right)\in P^{K_n, spt, asymm, 1}.
$

Dann gilt:
\begin{equation}
x_{\left\{n-1, n\right\}}+\sum_{u\in \left[n-2\right]} z_{\left(n, u\right), n-1} = 1. \label{eq:asymm_zusaetzlich_gueltig}
\end{equation}

Sei $\left(\begin{array}{c}
x \\
\widetilde{z}
\end{array}\right)\in P^{K_n, spt, symm, 0}$, wobei $\widetilde{z}_{\left(v, w\right), u} = z_{\left(v, w\right), u}$ für $\left(v, w, u\right)\in \left[n\right]^{\underline{3}^{1,2\not>3}}$ sei (mit anderen Worten: es ist möglich dem Punkt $\left(\begin{array}{c}
x \\
z
\end{array}\right)$ nach $P^{K_n, spt, symm, 0}$ "`hochzuliften"' und der hochgeliftete Punkt sei $\left(\begin{array}{c}
x \\
\widetilde{z}
\end{array}\right)$).
...
Dann gilt:
\begin{eqnarray}
\widetilde{z}_{\left(n-1, n\right), n-2} & = & 1 - x_{\left\{n-2, n-1\right\}} - \sum_{u\in \left[n-3\right]} z_{\left(n-1, u\right), n-2} \label{eq:z_n-1_n_n-2} \\
\widetilde{z}_{\left(n, n-1\right), n-2} & = & 1 -x_{\left\{n-2, n\right\}} - \sum_{u\in \left[n-3\right]} z_{\left(n, u\right), n-2}. \label{eq:z_n_n-1_n-2}
\end{eqnarray}
Wenn wir für ein $\left(\begin{array}{c}
x \\
z
\end{array}\right)\in P^{K_n, spt, asymm, 1}
$ gemäß dieser Konstruktion $\widetilde{z}_{\left(n-1, n\right), n-2}$  und $\widetilde{z}_{\left(n, n-1\right), n-2}$ bauen, so erfüllt dieses die für $P^{K_n, spt, symm, 0}$ gültigen Gleichungen
\begin{eqnarray}
x_{\left\{n-1, n\right\}} - \widetilde{z}_{\left(n-1, n\right), n-2}  - \widetilde{z}_{\left(n, n-1\right), n-2}
& \stackrel{\eqref{eq:z_n-1_n_n-2}, \eqref{eq:z_n_n-1_n-2}}{=} & 
x_{\left\{n-1, n\right\}} \nonumber \\
& & - 1 + x_{\left\{n-2, n-1\right\}} + \sum_{u\in \left[n-3\right]} z_{\left(n-1, u\right), n-2} \nonumber \\
& & - 1 + x_{\left\{n-2, n\right\}} + \sum_{u\in \left[n-3\right]} z_{\left(n, u\right), n-2} \nonumber \\
& = & 0, \label{eq:z_n-1_n_n-2_plus_z_n_n-1_n-2}
\end{eqnarray}
sowie für $n\geq 4$\footnote{Diese Einschränkung wird ausschließlich deswegen vorgenommen, weil für $n\leq 2$ die Ungleichung nicht sinnvoll ist und für $n=3$ die stärkeren Ungleichungen
\begin{eqnarray*}
\widetilde{z}_{\left(n-1, n\right), n-2} & \geq & 0 \\
\widetilde{z}_{\left(n, n-1\right), n-2} & \geq & 0
\end{eqnarray*}
gelten (vgl. Beweis von Satz \ref{sa:proj_P_p^2_neq_P_p^asymm}).} haben wir
\begin{eqnarray}
\widetilde{z}_{\left(n-1, n\right), n-2} & \geq & -1 \label{eq:z_n-1_n_n-2_geq-1} \\
\widetilde{z}_{\left(n, n-1\right), n-2} & \geq & -1. \label{eq:z_n_n-1_n-2_geq-1}
\end{eqnarray}
Der Schlupf in diesen Ungleichungen beträgt
\begin{displaymath}
x_{\left\{n-2, n\right\}} + x_{\left\{n-1, n\right\}} + \sum_{u\in \left[n-3\right]} z_{\left(n, u\right), n-2}
\end{displaymath}
bzw.
\begin{displaymath}
x_{\left\{n-2, n-1\right\}} + x_{\left\{n-1, n\right\}} + \sum_{u\in \left[n-3\right]} z_{\left(n-1, u\right), n-2}.
\end{displaymath}

Wegen $x, z\geq 0$ bedeutet dies: Gleichheit in diesen Ungleichungen ist für $\left(\begin{array}{c}
x \\
z
\end{array}\right)\in P^{K_n, spt, asymm, 1}$ genau dann erfüllt, wenn
\begin{eqnarray*}
x_{\left\{n-2, n\right\}} & = & 0 \\
x_{\left\{n-1, n\right\}} & = & 0 \\
z_{\left(n, u\right), n-2} & = & 0\ \forall u\in \left[n-3\right]
\end{eqnarray*}
bzw.
\begin{eqnarray*}
x_{\left\{n-2, n-1\right\}} & = & 0 \\
x_{\left\{n-1, n\right\}} & = & 0 \\
z_{\left(n-1, u\right), n-2} & = & 0\ \forall u\in \left[n-3\right].
\end{eqnarray*}
gilt.
\end{Sa}

\begin{Bem}
Sollte es uns also gelingen, für $n \geq 4$ einen Punkt aus $P^{K_n, spt, asymm, 1}$ zu finden, welcher die die Ungleichung \eqref{eq:z_n-1_n_n-2_geq-1} oder \eqref{eq:z_n_n-1_n-2_geq-1} mit Gleichheit erfüllt (wofür Satz \ref{sa:P_p^asymm_vs_P_p^2} eine notwendige und hinreichende Bedingung enthält), so hätten wir \eqref{eq:lift_polytop} widerlegt. Dies wird weiter hinten unsere Beweismethode darstellen.
\end{Bem}

\begin{bew}
Zu Gleichung \eqref{eq:asymm_zusaetzlich_gueltig}:
\begin{displaymath}
x_{\left\{n-1, n\right\}}+\sum_{u\in \left[n-2\right]} z_{\left(n, u\right), n-1} = 1.
\end{displaymath}

{
\allowdisplaybreaks
\begin{eqnarray*}
x_{\left\{n-1, n\right\}}+\sum_{u\in \left[n-2\right]} z_{\left(n, u\right), n-1}
& \stackrel{\eqref{eq:spt_erweitert_asymm_prim_1}}{=} & x_{\left\{n-1, n\right\}}+\sum_{u\in \left[n-2\right]} \left(x_{\left\{u, n\right\}} - z_{\left(u, n\right), n-1}\right) \\
& = & \sum_{u\in \left[n-1\right]} x_{\left\{u, n\right\}} + \sum_{u\in \left[n-2\right]} \left(- z_{\left(u, n\right), n-1}\right) \\
& \stackrel{\eqref{eq:spt_erweitert_asymm_prim_2}}{=}  & \sum_{u\in \left[n-1\right]} \left(1-\sum_{\widetilde{u}\in \left[n-1\right]\backslash \left\{u\right\}} z_{\left(u, \widetilde{u}\right), n} \right) \\
& & + \sum_{u\in \left[n-2\right]} \left( -1 + x_{\left\{u, n-1\right\}} + \sum_{\widetilde{u} \in \left[n-2\right] \backslash \left\{u\right\}} z_{\left(u, \widetilde{u}\right), n-1} \right) \\
& = & 1 -\sum_{\left(u, \widetilde{u}\right)\in \left[n-1\right]^{\underline{2}}} z_{\left(u, \widetilde{u}\right), n} \\
& & + \sum_{\left(u, \widetilde{u}\right)\in \left[n-2\right]^{\underline{2}}} z_{\left(u, \widetilde{u}\right), n-1} + \sum_{u\in \left[n-2\right]} x_{\left\{u, n-1\right\}} \\
& \stackrel{\eqref{eq:spt_erweitert_asymm_prim_1}}{=} & 1 - \sum_{\left\{u, \widetilde{u}\right\}\in {\left[n-1\right] \choose 2}} x_{\left\{u, \widetilde{u}\right\}} \\
& & + \sum_{\left\{u, \widetilde{u}\right\}\in {\left[n-2\right] \choose 2}} x_{\left\{u, \widetilde{u}\right\}} + \sum_{u\in \left[n-2\right]} x_{\left\{u, n-1\right\}} \\
& = & 1.
\end{eqnarray*}
}

Aus der für $P^{K_n, spt, symm, 0}$ gültigen Gleichung \eqref{spt_erweitert_primal2_2} folgt:
\begin{eqnarray*}
\widetilde{z}_{\left(n-1, n\right), n-2} & = & 1 - x_{\left\{n-2, n-1\right\}} - \sum_{u\in \left[n-3\right]} z_{\left(n-1, u\right), n-2}  \\
\widetilde{z}_{\left(n, n-1\right), n-2} & = & 1 -x_{\left\{n-2, n\right\}} - \sum_{u\in \left[n-3\right]} z_{\left(n, u\right), n-2}.
\end{eqnarray*}
Somit muss, wenn wir $\left(\begin{array}{c}
x \\
z
\end{array}\right)\in P^{K_n, spt, asymm, 1}$ zu einem Punkt $\left(\begin{array}{c}
x \\
\widetilde{z}
\end{array}\right)\in P^{K_n, spt, symm, 0}$ hochliften, diese Gleichung erfüllt sein.

Zu Gleichung \eqref{eq:z_n-1_n_n-2_plus_z_n_n-1_n-2}:
\begin{eqnarray*}
x_{\left\{n-1, n\right\}} - \widetilde{z}_{\left(n-1, n\right), n-2}  - \widetilde{z}_{\left(n, n-1\right), n-2}
& = & 
x_{\left\{n-1, n\right\}} \\
...
\allowdisplaybreaks
\begin{eqnarray*}
x_{\left\{n-1, n\right\}} - \widetilde{z}_{\left(n-1, n\right), n-2}  & & \\
- \widetilde{z}_{\left(n, n-1\right), n-2}
& = & 
x_{\left\{n-1, n\right\}} \\
& & - 1 + x_{\left\{n-2, n-1\right\}} + \sum_{u\in \left[n-3\right]} z_{\left(n-1, u\right), n-2} \\
& & - 1 + x_{\left\{n-2, n\right\}} + \sum_{u\in \left[n-3\right]} z_{\left(n, u\right), n-2} \\
& = & x_{\left\{n-1, n\right\}} + x_{\left\{n-2, n-1\right\}} + x_{\left\{n-2, n\right\}} -2 \\
& & + \sum_{u\in \left[n-3\right]} z_{\left(n-1, u\right), n-2} + \sum_{u\in \left[n-3\right]} z_{\left(n, u\right), n-2} \\
& \stackrel{\eqref{eq:spt_erweitert_asymm_prim_1}}{=} & x_{\left\{n-1, n\right\}} + x_{\left\{n-2, n-1\right\}} + x_{\left\{n-2, n\right\}} -2 \\
& & + \sum_{u\in \left[n-3\right]} \left(x_{\left\{u, n-1\right\}} - z_{\left(u, n-1\right), n-2}\right) \\
& & + \sum_{u\in \left[n-3\right]} \left(x_{\left\{u, n\right\}} - z_{\left(u, n\right), n-2}\right) \\
& = & x_{\left\{n-1, n\right\}} + x_{\left\{n-2, n-1\right\}} + x_{\left\{n-2, n\right\}} -2 \\
& & + \sum_{u\in \left[n-3\right]} \left(x_{\left\{u, n-1\right\}} + x_{\left\{u, n\right\}} \right) \\
& & + \sum_{u\in \left[n-3\right]} \left( - z_{\left(u, n-1\right), n-2} - z_{\left(u, n\right), n-2}\right) \\
& \stackrel{\eqref{eq:spt_erweitert_asymm_prim_2}}{=} & x_{\left\{n-1, n\right\}} + x_{\left\{n-2, n-1\right\}} + x_{\left\{n-2, n\right\}} -2 \\
& & + \sum_{u\in \left[n-3\right]} \left(x_{\left\{u, n-1\right\}} + x_{\left\{u, n\right\}} \right) \\
& & + \sum_{u\in \left[n-3\right]} \left(-1+x_{\left\{u, n-2\right\}} +\sum_{\widetilde{u} \in \left[n-3\right] \backslash \left\{u\right\}} z_{\left(u, \widetilde{u}\right), n-2} \right) \\
& = &  -\left(n-1\right) + x_{\left\{n-1, n\right\}} + x_{\left\{n-2, n-1\right\}} + x_{\left\{n-2, n\right\}} \\
& & + \sum_{u\in \left[n-3\right]} \left(x_{\left\{u, n-2\right\}} + x_{\left\{u, n-1\right\}} + x_{\left\{u, n\right\}} \right) \\
& & + \sum_{\left(u, \widetilde{u}\right)\in \left[n-3\right]^{\underline{2}}} z_{\left(u, \widetilde{u}\right), n-2} \\
& \stackrel{\eqref{eq:spt_erweitert_asymm_prim_1}}{=} & -\left(n-1\right) + x_{\left\{n-1, n\right\}} + x_{\left\{n-2, n-1\right\}} + x_{\left\{n-2, n\right\}}  \\
& & + \sum_{u\in \left[n-3\right]} \left(x_{\left\{u, n-2\right\}} + x_{\left\{u, n-1\right\}} + x_{\left\{u, n\right\}} \right) \\
& & + \sum_{\left\{u, \widetilde{u}\right\} \in {\left[n-3\right] \choose 2}} x_{\left\{u, \widetilde{u}\right\}} \\
& = & -\left(n-1\right) + \sum_{\left\{u, \widetilde{u}\right\} \in {\left[n\right] \choose 2}} x_{\left\{u, \widetilde{u}\right\}}
\end{eqnarray*}
}

Da wir aus dem Beweis von Satz \ref{sa:spt_erw_asymmetrisch} wissen, dass
\begin{displaymath}
\sum_{\left\{u, \widetilde{u}\right\} \in {\left[n\right] \choose 2}} x_{\left\{u, \widetilde{u}\right\}} = n-1
\end{displaymath}
ist, folgt die Aussage. \\

Zu Ungleichung \eqref{eq:z_n-1_n_n-2_geq-1}: $\widetilde{z}_{\left(n-1, n\right), n-2} \geq -1$:
{
\allowdisplaybreaks
\begin{eqnarray*}
\widetilde{z}_{\left(n-1, n\right), n-2} & = & 1 - x_{\left\{n-2, n-1\right\}} - \sum_{u\in \left[n-3\right]} z_{\left(n-1, u\right), n-2} \\
& \stackrel{\eqref{eq:spt_erweitert_asymm_prim_1}}{=} & 1 - x_{\left\{n-2, n-1\right\}} - \sum_{u\in \left[n-3\right]} \left(x_{\left\{u, n-1\right\}} - z_{\left(u, n-1\right), n-2}\right) \\
& = & 1 - \sum_{u\in \left[n-2\right]} x_{\left\{u, n-1\right\}} + \sum_{u\in \left[n-3\right]} z_{\left(u, n-1\right), n-2} \\
& \stackrel{\eqref{eq:spt_erweitert_asymm_prim_2}}{=}  & 1 - \sum_{u\in \left[n-2\right]} x_{\left\{u, n-1\right\}} \\
& & + \sum_{u\in \left[n-3\right]} \left(1 - x_{\left\{u, n-2\right\}} - z_{\left(u, n\right), n-2}- \sum_{\widetilde{u}\in \left[n-3\right]\backslash \left\{u\right\}} z_{\left(u, \widetilde{u}\right), n-2}\right) \\
& = & n-2 - \sum_{u\in \left[n-2\right]} x_{\left\{u, n-1\right\}} \\
& & + \sum_{u\in \left[n-3\right]} \left( - x_{\left\{u, n-2\right\}} - z_{\left(u, n\right), n-2}- \sum_{\widetilde{u}\in \left[n-3\right]\backslash \left\{u\right\}} z_{\left(u, \widetilde{u}\right), n-2}\right) \\
& \stackrel{\eqref{eq:spt_erweitert_asymm_prim_1}}{=} & n-2 - \sum_{u\in \left[n-2\right]} x_{\left\{u, n-1\right\}} \\
& & + \sum_{u\in \left[n-3\right]} \left( - x_{\left\{u, n-2\right\}} - x_{\left\{u, n\right\}} + z_{\left(n, u\right), n-2} \right) - \sum_{\left\{u, \widetilde{u} \right\} \in {\left[n-3\right] \choose 2}} x_{\left\{u, \widetilde{u}\right\}} \\
& = & n-2 - \sum_{\left\{u, \widetilde{u}\right\} \in {\left[n\right] \choose 2}} x_{\left\{u, \widetilde{u}\right\}} + x_{\left\{n-2, n\right\}} + x_{\left\{n-1, n\right\}} + \sum_{u\in \left[n-3\right]} z_{\left(n, u\right), n-2} \\
& \stackrel{\textnormal{Satz \ref{sa:spt_erw_asymmetrisch}}}{=} & -1 + x_{\left\{n-2, n\right\}} + x_{\left\{n-1, n\right\}} + \sum_{u\in \left[n-3\right]} z_{\left(n, u\right), n-2} \\
& \stackrel{x, z\geq 0}{\geq} & -1.
\end{eqnarray*}
}

Zu Ungleichung \eqref{eq:z_n_n-1_n-2_geq-1}: $\widetilde{z}_{\left(n, n-1\right), n-2} \geq -1$:
{
\allowdisplaybreaks
\begin{eqnarray*}
\widetilde{z}_{\left(n, n-1\right), n-2} & = & 1 -x_{\left\{n-2, n\right\}} - \sum_{u\in \left[n-3\right]} z_{\left(n, u\right), n-2} \\
& \stackrel{\eqref{eq:spt_erweitert_asymm_prim_1}}{=} & 1 - x_{\left\{n-2, n\right\}} - \sum_{u\in \left[n-3\right]} \left(x_{\left\{u, n\right\}} - z_{\left(u, n\right), n-2}\right) \\
& = & 1 - \sum_{u\in \left[n-2\right]} x_{\left\{u, n\right\}} + \sum_{u\in \left[n-3\right]} z_{\left(u, n\right), n-2} \\
& \stackrel{\eqref{eq:spt_erweitert_asymm_prim_2}}{=} & 1 - \sum_{u\in \left[n-2\right]} x_{\left\{u, n\right\}} \\
& & + \sum_{u\in \left[n-3\right]} \left(1 - x_{\left\{u, n-2\right\}} - z_{\left(u, n-1\right), n-2}- \sum_{\widetilde{u}\in \left[n-3\right]\backslash \left\{u\right\}} z_{\left(u, \widetilde{u}\right), n-2}\right) \\
& = & n-2 - \sum_{u\in \left[n-2\right]} x_{\left\{u, n\right\}} \\
& & + \sum_{u\in \left[n-3\right]} \left( - x_{\left\{u, n-2\right\}} - z_{\left(u, n-1\right), n-2}- \sum_{\widetilde{u}\in \left[n-3\right]\backslash \left\{u\right\}} z_{\left(u, \widetilde{u}\right), n-2}\right) \\
& \stackrel{\eqref{eq:spt_erweitert_asymm_prim_1}}{=} & n-2 - \sum_{u\in \left[n-2\right]} x_{\left\{u, n\right\}} \\
& & + \sum_{u\in \left[n-3\right]}  \left( - x_{\left\{u, n-2\right\}} - x_{\left\{u, n-1\right\}} + z_{\left(n-1, u\right), n-2} \right) - \sum_{\left\{u, \widetilde{u}\right\} \in {\left[n-3\right] \choose 2}} x_{\left\{u, \widetilde{u}\right\}} \\
& = & n-2 - \sum_{\left(u, \widetilde{u}\right) \in \left[n\right]^{\underline{2}}} x_{\left\{u, \widetilde{u}\right\}} + x_{\left\{n-2, n-1\right\}} + x_{\left\{n-1, n\right\}} + \sum_{u\in \left[n-3\right]} z_{\left(n-1, u\right), n-2} \\
& \stackrel{\textnormal{Satz \ref{sa:spt_erw_asymmetrisch}}}{=} & -1 +  x_{\left\{n-2, n-1\right\}} + x_{\left\{n-1, n\right\}} + \sum_{u\in \left[n-3\right]} z_{\left(n-1, u\right), n-2} \\
& \stackrel{x, z\geq 0}{\geq} & -1.
\end{eqnarray*}
}
\end{bew}

Nun wollen wir die Vermutung
\begin{displaymath}
\proj_{\mathbb{R}^{\left[n\right] \choose 2} \times \mathbb{R}^{\left[n\right]^{\underline{3}^{1,2\not>3}}}} P^{K_n, spt, symm, 0} = P^{K_n, spt, asymm, 1}.
\end{displaymath}
für $n\geq 4$ widerlegen und im Anschluss für $n\leq 3$ zeigen.

\begin{Le}
\label{le:keine_proj_P_p^2_P_p^asymm_1}
Für $n\geq 4$ existiert ein $\left(
\begin{array}{c}
x \\
z
\end{array}
\right) \in P^{K_n, spt, asymm, 1}$ mit 
\begin{displaymath}
1 - x_{\left\{n-2, n-1\right\}} - \sum_{u\in \left[n-3\right]} z_{\left(n-1, u\right), n-2} = -1.
\end{displaymath}
\end{Le}
\begin{bew}
Für $n=4$ setze
\begin{eqnarray*}
x_{\left\{v, w\right\}} & := & \begin{cases}
1 & \left\{v, w\right\} \in \left\{\left\{1, 3\right\}, \left\{1, 4\right\}, \left\{2, 3\right\}\right\} \\
0 & \textnormal{sonst}
\end{cases} \\
z_{\left(v, w\right), u} & := & \begin{cases}
1 & \left(v, w, u\right) \in \left\{\left(1, 4, 2\right), \left(2, 3, 4\right), \left(3, 1, 2\right), \left(3, 1, 4\right), \left(4, 1, 3\right)\right\} \\
0 & \textnormal{sonst.}
\end{cases}
\end{eqnarray*}

Nach Betrachtung dieses Sonderfalls können wir voraussetzen, dass $n\geq 5$ ist.

Setze für $\left\{v, w\right\}\in {\left[n\right] \choose 2}$ bzw. $\left(v, w, u\right) \in \left[n\right]^{\underline{3}^{1, 2 \not> 3}}$:
{
\allowdisplaybreaks
\begin{eqnarray*}
x_{\left\{v, w\right\}} & := & \begin{cases}
\frac{1}{2} & \left\{v, w\right\} \in \left\{\left\{1, 2\right\}, \left\{1, n-2\right\}, \left\{1, n\right\}\right\} \\
1 & \left\{v, w\right\} = \left\{1, u\right\} \textnormal{ mit } u\notin \left\{2, n-2, n\right\} \\
\frac{1}{2} & \left\{v, w\right\} \in \left\{\left\{2, n-1\right\}, \left\{2, n\right\}\right\} \\
\frac{1}{2} & \left\{v, w\right\} = \left\{n-2, n-1\right\} \\
0 & \textnormal{sonst}
\end{cases} \\
z_{\left(v, w\right), u} & := & \begin{cases}
\frac{1}{2} & \left(v, w, u\right) = \left(1, n-1, n\right) \\
\frac{1}{2} & \left(v, w, u\right) = \left(1, n, 2\right) \\
\frac{1}{2} & \left(v, w, u\right) = \left(1, n, n-2\right) \\
\frac{1}{2} & \left(v, w\right) = \left(2, 1\right) \\
\frac{1}{2} & \left(v, w\right) = \left(2, n\right) \wedge u\neq n-1 \\
1 & 3 \leq v \leq n-3 \wedge w = 1 \\
\frac{1}{2} & \left(v, w\right) = \left(n-2, 1\right) \\
\frac{1}{2} & \left(v, w, u\right) = \left(n-2, n-1, n\right) \\
1 & \left(v, w\right) = \left(n-1, 1\right) \wedge u \neq n \\
\frac{1}{2} & \left(v, w, u\right) = \left(n-1, 1, n\right) \\
\frac{1}{2} & \left(v, w\right) = \left(n-1, 2\right) \\
\frac{1}{2} & \left(v, w\right) = \left(n, 1\right) \wedge u\geq 3 \wedge u \neq n-2 \\
\frac{1}{2} & \left(v, w, u\right) = \left(n, 2, n-1\right) \\
0 & \textnormal{sonst.}
\end{cases}
\end{eqnarray*}
}

Es reicht aus, die Gültigkeit von \eqref{eq:spt_erweitert_asymm_prim_1} und \eqref{eq:spt_erweitert_asymm_prim_2} zu zeigen.

\paragraph{Zu \eqref{eq:spt_erweitert_asymm_prim_1}:} ~ \\
...
Sei daher $\left(\left\{v, w\right\}, u\right)\in EV_n^{\not>}$ -- ohne Einschränkung gelte im Folgenden $v<w$.

\subparagraph{Fall 1: $v = 1 \wedge w\in \left\{2, n-2\right\}$} Dann gilt $x_{\left\{1, w\right\}} = \frac{1}{2}$, $z_{\left(1, w\right), u} = 0$ und $z_{\left(w, 1\right), u} = \frac{1}{2}$.

\subparagraph{Fall 2: $\left(v, w\right) = \left(1, n-1\right)$} Es gilt $x_{\left\{1, n-1\right\}} = 1$. Falls $u=n$ ist, gilt $z_{\left(1, n-1\right), n} = z_{\left(n-1, 1\right), n} = \frac{1}{2}$. Für $u\neq n$ ist $z_{\left(1, n-1\right), u} = 0$ und $z_{\left(n-1, 1\right), u} = 1$.

\subparagraph{Fall 3: $\left(v, w\right) = \left(1, n\right) \wedge u\in \left\{2, n-2\right\}$} Dann gilt $x_{\left\{1, n\right\}} = \frac{1}{2}$, $z_{\left(1, n\right), u} = \frac{1}{2}$ und $z_{\left(n, 1\right), u} = 0$.

\subparagraph{Fall 4: $\left(v, w\right) = \left(1, n\right) \wedge u\notin \left\{2, n-2\right\}$} Dann ist sicher $u\geq 3$. Es gilt $x_{\left\{1, n\right\}} = \frac{1}{2}$, $z_{\left(1, n\right), u} = 0$ und $z_{\left(n, 1\right), u} = \frac{1}{2}$.

\subparagraph{Fall 5: $v = 1 \wedge w\notin \left\{2, n-2, n-1, n\right\}$} Dann gilt $x_{\left\{1, n\right\}} = 1$, $z_{\left(1, w\right), u} = 0$ und (weil sicher $w\geq 3$ und $w\neq n-1$ erfüllt ist) $z_{\left(w, 1\right), u} = 1$.

\subparagraph{Fall 6: $\left(v, w\right) = \left(2, n-1\right)$} Dann haben wir $x_{\left\{2, n-1\right\}} = \frac{1}{2}$, $z_{\left(2, n-1\right), u} = 0$ und $z_{\left(n-1, 2\right), u} = \frac{1}{2}$.

\subparagraph{Fall 7: $\left(v, w\right) = \left(2, n\right)$} Dann ist $x_{\left\{2, n\right\}} = \frac{1}{2}$. Falls $u\neq n-1$ ist, gilt: $z_{\left(2, n\right), u} = \frac{1}{2}$ und $z_{\left(n, 2\right), u} = 0$. Für $u=n-1$ ist $z_{\left(2, n\right), n-1} = 0$ und $z_{\left(n, 2\right), n-1} = \frac{1}{2}$.

\subparagraph{Fall 8: $\left(v, w, u\right) = \left(n-2, n-1, n\right)$} Dann ist $x_{\left\{n-2, n-1\right\}} = \frac{1}{2}$, $z_{\left(n-2, n-1\right), n} = \frac{1}{2}$ und $z_{\left(n-1, n-2\right), n} = \frac{1}{2}$.

\subparagraph{Fall 9: $v\notin \left\{1, 2, n-2\right\}$} In diesem Fall ist $x_{\left\{v, w\right\}} = 0$. Wegen $\left(\left\{v, w\right\}, u\right)\in EV_n^{\not>}$ gilt sicher $v\leq n-3$. Somit haben wir $z_{\left(v, w\right), u} = 0$ (weil wir oben $v<w$ vorausgesetzt haben, kann nicht der Fall "`$z_{\left(v, w\right), u} = 0$ für $3 \leq v \leq n-3 \wedge w = 1$"' eintreten).

Es bleibt also sicherzustellen, dass auch $z_{\left(w, v\right), u} = 0$ ist.

Es gilt sicher $v\geq 3$ und wegen der Annahme $v<w$ ist $w\geq 4$. Man überzeugt sich anhand der Definition von $z$, dass hier in der Tat $z_{\left(w, v\right), u} = 0$ erfüllt ist.

\paragraph{Zu \eqref{eq:spt_erweitert_asymm_prim_2}:} ~ \\
...
Sei daher im Folgenden $\left(v, w\right)\in {[n]^{\underline{2}}}^<$.

\subparagraph{Fall 1: $v = 1 \wedge w \in \left\{2, n-2, n\right\}$} Hier gilt $x_{\left\{1, w\right\}} = \frac{1}{2}$ und von den $z_{\left(v, u\right), w}$ ist als einziges $z_{ \left(1, n\right), 2}$ bzw. $z_{ \left(1, n\right), n-2}$ bzw. $z_{ \left(1, n-1\right), n}$ gleich $\frac{1}{2}$ (die anderen sind $0$).

\subparagraph{Fall 2: $v = 1 \wedge w \notin \left\{2, n-2, n\right\}$} Dann ist $x_{\left\{1, w\right\}} = 1$ und es gilt $z_{\left(v, u\right), w} = 0$ für alle $u \in \left[n\right]$ mit $\left(v, u, w\right) \in \left[n\right]^{\underline{3}^{1, 2 \not> 3}}$.

\subparagraph{Fall 3: $v = 2 \wedge w \in \left\{n-1, n\right\}$} Dann gilt $x_{\left\{2, w\right\}} = \frac{1}{2}$ und von den $z_{\left(v, u\right), w}$ ist als einziges $z_{ \left(2, 1\right), w}=\frac{1}{2}$ (die anderen sind $0$).

\subparagraph{Fall 4: $v = 2 \wedge w \notin \left\{n-1, n\right\}$} Dann gilt $x_{\left\{2, w\right\}} = 0$ und für die $z_{\left(v, u\right), w}$ gilt: $z_{ \left(2, 1\right), w}=z_{ \left(2, n\right), w}=\frac{1}{2}$ (die anderen $z_{\left(v, u\right), w}$ sind $0$).

\subparagraph{Fall 5: $3 \leq v \leq n-3$} In diesem Fall haben wir $x_{\left\{v, w\right\}} = 0$ und genau für $u = 1$ gilt $z_{\left(v, u\right), w} = 1$ (sonst 0).

\subparagraph{Fall 6: $\left(v, w\right) = \left(n-2, n-1\right)$} Dann ist $x_{\left\{n-2, n-1\right\}} = \frac{1}{2}$ und von den $z_{\left(v, u\right), w}$ ist nur $z_{\left(n-2, 1\right), n-1} = \frac{1}{2}$ (alle sonstigen  $z_{\left(v, u\right), w}$ nehmen den Wert $0$ an).

\subparagraph{Fall 7: $\left(v, w\right) = \left(n-2, n\right)$} Hier ist $x_{\left\{n-2, n\right\}} = 0$. Für die $z_{\left(v, u\right), w}$ gilt: $z_{\left(n-2, n-1\right), n} = z_{\left(n-2, 2\right), n} = \frac{1}{2}$ (die restlichen sind $0$).

\subparagraph{Fall 8: $\left(v, w\right) = \left(n-1, n\right)$} Hier ist $x_{\left\{n-1, n\right\}} = 0$. Für die $z_{\left(v, u\right), w}$ gilt: $z_{\left(n-1, 1\right), n} = z_{\left(n-2, 1\right), n} = \frac{1}{2}$ (die restlichen sind $0$).
\end{bew}

\begin{Bem}
\label{bem:keine_proj_P_p^2_P_p^asymm_1}
Ein weiterer Punkt für $n=5$, der ebenfalls die Gleichung $1 - x_{\left\{n-2, n-1\right\}} - \sum_{u\in \left[n-3\right]} z_{\left(n-1, u\right), n-2} = -1$ erfüllt, aber dessen sämtliche Komponenten $0$-$1$-wertig sind, ist
\begin{eqnarray*}
x_{\left\{v, w\right\}} & := & \begin{cases}
1 & \left\{v, w\right\} \in \left\{\left\{1, 2\right\}, \left\{1, 4\right\}, \left\{1, 5\right\}, \left\{3, 4\right\}\right\} \\
0 & \textnormal{sonst}
\end{cases} \\
z_{\left(v, w\right), u} & := & \begin{cases}
1 & \left(v, w, u\right) \in \parbox[t]{10cm}{$\left\{\left(1, 5, 3\right), \left(2, 1, 3\right), \left(2, 1, 4\right), \left(2, 1, 5\right), \left(3, 4, 5\right), \right.$ \\ $\left.\left(4, 1, 2\right), \left(4, 1, 3\right), \left(4, 1, 5\right), \left(5, 1, 2\right), \left(5, 1, 4\right)\right\}$} \\
0 & \textnormal{sonst.}
\end{cases}
\end{eqnarray*}
\end{Bem}
...
\begin{Le}
\label{le:keine_proj_P_p^2_P_p^asymm_2}
Für $n\geq 4$ existiert ein 0/1-wertiges $\left(
\begin{array}{c}
x \\
z
\end{array}
\right) \in P^{K_n, spt, asymm, 1}$ mit 
\begin{displaymath}
1 -x_{\left\{n-2, n\right\}} - \sum_{u\in \left[n-3\right]} z_{\left(n, u\right), n-2}, = -1.
\end{displaymath}
\end{Le}
\begin{bew}
Für $n=4$ setze
\begin{eqnarray*}
x_{\left\{v, w\right\}} & := & \begin{cases}
1 & \left\{v, w\right\} \in \left\{\left\{1, 3\right\}, \left\{1, 4\right\}, \left\{2, 4\right\}\right\} \\
0 & \textnormal{sonst}
\end{cases} \\
z_{\left(v, w\right), u} & := & \begin{cases}
1 & \left(v, w, u\right) \in \left\{\left(1, 3, 2\right), \left(2, 4, 3\right), \left(3, 1, 4\right), \left(4, 1, 2\right), \left(4, 1, 3\right)\right\} \\
0 & \textnormal{sonst}
\end{cases}
\end{eqnarray*}

Für $n\geq 5$ setze für $\left\{v, w\right\}\in {\left[n\right] \choose 2}$ bzw. $\left(v, w, u\right) \in \left[n\right]^{\underline{3}^{1, 2 \not> 3}}$:
{
\allowdisplaybreaks
\begin{eqnarray*}
x_{\left\{v, w\right\}} & := & \begin{cases}
1 & \left\{v, w\right\} = \left\{1, u\right\} \textnormal{ mit } u\notin \left\{2, n-1\right\} \\
1 & \left\{v, w\right\} \in \left\{\left\{2, n-1\right\}, \left\{2, n\right\}\right\} \\
0 & \textnormal{sonst}
\end{cases} \\
z_{\left(v, w\right), u} & := & \begin{cases}
1 & \left(v, w\right) = \left(1, n\right) \wedge u\in \left\{2, n-1\right\} \\
1 & \left(v, w, u\right) = \left(2, n-1, n-2\right) \\
1 & \left(v, w\right) = \left(2, n\right) \wedge u \notin \left\{n-2, n-1\right\} \\
1 & 3 \leq v \leq n-2 \wedge w = 1 \\
1 & \left(v, w\right) = \left(n-1, 2\right) \wedge u\neq n-2 \\
1 & \left(v, w\right) = \left(n, 1\right) \wedge u \notin \left\{2, n-1\right\} \\
1 & \left(v, w, u\right) = \left(n, 2\right) \wedge u \in \left\{n-2, n-1\right\} \\
0 & \textnormal{sonst}
\end{cases}
\end{eqnarray*}
}
...
\subparagraph{Fall 1: $v = 1 \wedge w\in \left\{2, n-1\right\}$} Dann gilt $x_{\left\{1, w\right\}} = z_{\left(1, w\right), u} = z_{\left(w, 1\right), u} = 0$.

\subparagraph{Fall 2: $\left(v, w\right) = \left(1, n\right) \wedge u\in \left\{2, n-1\right\}$} Dann gilt $x_{\left\{1, n\right\}} = 1$, $z_{\left(1, n\right), u} = 1$ und $z_{\left(n, 1\right), u} = 0$.

\subparagraph{Fall 3: $\left(v, w\right) = \left(1, n\right) \wedge  u\notin \left\{2, n-1\right\}$} Hier haben wir $x_{\left\{1, n\right\}} = 1$, $z_{\left(1, n\right), u} = 0$ und $z_{\left(n, 1\right), u} = 1$.

\subparagraph{Fall 4: $v = 1 \wedge w\notin \left\{2, n-1,n\right\}$} Dann gilt $x_{\left\{1, w\right\}} = 1$, $z_{\left(1, w\right), u} = 0$ und (weil sicher $3 \leq w \leq n-2$ erfüllt ist) $z_{\left(w, 1\right), u} = 1$.

\subparagraph{Fall 5: $\left(v, w\right) = \left(2, n-1\right)$} Dann haben wir $x_{\left\{2, n-1\right\}} = 1$, Falls $u=n-2$ ist, gilt $z_{\left(2, n-1\right), n-2} = 1$ und  $z_{\left(n-1, 2\right), n-2} = 0$. Sonst ist $z_{\left(2, n-1\right), u} = 0$ und $z_{\left(n-1, 2\right), u} = 1$.

\subparagraph{Fall 6: $\left(v, w\right) = \left(2, n\right)$} Dann ist $x_{\left\{2, n\right\}} = 1$. Falls $u\notin \left\{n-2, n-1\right\}$ ist, gilt: $z_{\left(2, n\right), u} = 1$ und $z_{\left(n, 2\right), u} = 0$. Für $u \in \left\{n-2, n-1\right\}$ ist $z_{\left(2, n\right), u} = 0$ und $z_{\left(n, 2\right), u} = 1$.

\subparagraph{Fall 7: $v = 2 \wedge w \leq n-2$} Es gilt $x_{\left\{2, w\right\}} = 0$, $z_{\left(2, w\right), u} = 0$ und $z_{\left(w, 2\right), u} = 0$.

\subparagraph{Fall 8: $v \geq 3$} In diesem Fall ist $x_{\left\{v, w\right\}} = 0$. Wegen $v<w$ ist sicher $z_{\left(v, w\right), u} = z_{\left(w, v\right), u} = 0$.
...
\subparagraph{Fall 1: $v = 1 \wedge w \in \left\{2, n-1\right\}$} Hier gilt $x_{\left\{1, w\right\}} = 0$ und von den $z_{\left(v, u\right), w}$ ist als einziges $z_{ \left(1, n\right), w}$ gleich $1$ (die anderen sind $0$).

\subparagraph{Fall 2: $v = 1 \wedge w \notin \left\{2, n-1\right\}$} Dann ist $x_{\left\{1, w\right\}} = 1$ und $z_{\left(v, u\right), w}$ ist $0$ für alle $u$ mit $\left(1, u, w\right) \in \left[n\right]^{\underline{3}^{1, 2 \not> 3}}$.

\subparagraph{Fall 3: $v = 2 \wedge w \in \left\{n-1, n\right\}$} Dann gilt $x_{\left\{2, w\right\}} = 1$ und die $z_{\left(v, u\right), w}$ sind alle gleich $0$.

\subparagraph{Fall 4: $v = 2 \wedge w \notin \left\{n-1, n\right\}$} Dann gilt $x_{\left\{2, w\right\}} = 0$ und für die $z_{\left(v, u\right), w}$ gilt:

Falls $w=n-2$ ist, gilt $z_{ \left(2, n-1\right), n-2} = 1$ und sonst $z_{ \left(2, n-1\right), n-2} = 0$.

Für $w\neq n-2$ ist $z_{\left(2, n\right), w}=1$ und die anderen $z_{\left(2, u\right), w}$ sind $0$.

\subparagraph{Fall 5: $3 \leq v \leq n-2$} In diesem Fall haben wir $x_{\left\{v, w\right\}} = 0$ und genau für $u = 1$ gilt $z_{\left(v, u\right), w} = 1$ (sonst 0).

\subparagraph{Fall 6: $\left(v, w\right) = \left(n-1, n\right)$} Dann ist $x_{\left\{n-1, n\right\}} = 0$ und von den $z_{\left(n-1, u\right), n}$ ist nur $z_{\left(n-1, 2\right), n} =1$ (der Rest $0$).
\end{bew}

\begin{Bem}
\label{bem:keine_proj_P_p^2_P_p^asymm_2}
Ein weiteres Beispiel für $n = 5$ mit $1 -x_{\left\{n-2, n\right\}} - \sum_{u\in \left[n-3\right]} z_{\left(n, u\right), n-2}, = -1$ ist
\begin{eqnarray*}
x_{\left\{v, w\right\}} & := & \begin{cases}
1 & \left\{v, w\right\} \in \left\{\left\{1, 2\right\}, \left\{1, 4\right\}, \left\{1, 5\right\}, \left\{3, 5\right\}\right\} \\
0 & \textnormal{sonst}
\end{cases} \\
z_{\left(v, w\right), u} & := & \begin{cases}
1 & \left(v, w, u\right) \in \parbox[t]{10cm}{$\left\{\left(1, 4, 3\right), \left(2, 1, 3\right), \left(2, 1, 4\right), \left(2, 1, 5\right), \left(3, 5, 4\right), \right.$ \\ $\left.\left(4, 1, 2\right), \left(4, 1, 5\right), \left(5, 1, 2\right), \left(5, 1, 3\right), \left(5, 1, 4\right)\right\}$} \\
0 & \textnormal{sonst.}
\end{cases}
\end{eqnarray*}
\end{Bem}
...
\begin{Sa}
\label{sa:proj_P_p^2_neq_P_p^asymm}
Für $n\geq 4$ gilt:
\begin{displaymath}
\proj_{\mathbb{R}^{\left[n\right] \choose 2} \times \mathbb{R}^{\left[n\right]^{\underline{3}^{1,2\not>3}}}} P^{K_n, spt, symm, 0} \neq P^{K_n, spt, asymm, 1}.
\end{displaymath}
...
\end{array}
\right)
\in P^{K_n, spt, asymm, 1}$ und $\left(
\begin{array}{c}
x \\
\widetilde{z}
\end{array}
\right)
\in P^{K_n, spt, symm, 0}$ mit $\widetilde{z}_{\left(v_1, v_2\right), v_3} = z_{\left(v_1, v_2\right), v_3}$ für $\left(v_1, v_2, v_3\right) \in \left[n\right]^{\underline{3}^{1,2\not>3}}$.

Wegen \eqref{eq:z_n-1_n_n-2} und \eqref{eq:z_n_n-1_n-2} gilt
\begin{eqnarray*}
\widetilde{z}_{\left(n-1, n\right), n-2} & = & 1 - x_{\left\{n-2, n-1\right\}} - \sum_{u\in \left[n-3\right]} z_{\left(n-1, u\right), n-2} \\
\widetilde{z}_{\left(n, n-1\right), n-2} & = & 1 -x_{\left\{n-2, n\right\}} - \sum_{u\in \left[n-3\right]} z_{\left(n, u\right), n-2}.
\end{eqnarray*}

Nach Lemmata \ref{le:keine_proj_P_p^2_P_p^asymm_1} und \ref{le:keine_proj_P_p^2_P_p^asymm_2} gibt es für $n\geq 4$ einen Punkt aus $P^{K_n, spt, asymm, 1}$, für den diese Terme negativ werden. Da jedoch
\begin{eqnarray*}
\widetilde{z}_{\left(n-1, n\right), n-2} & \geq & 0 \\
\widetilde{z}_{\left(n, n-1\right), n-2} & \geq & 0
\end{eqnarray*}
erfüllt ist, haben wir einen Widerspruch zur Annahme, dass 
\begin{displaymath}
\proj_{\mathbb{R}^{\left[n\right] \choose 2} \times \mathbb{R}^{\left[n\right]^{\underline{3}^{1,2\not>3}}}} P^{K_n, spt, symm, 0} = P^{K_n, spt, asymm, 1}
\end{displaymath}
gilt.
...
\proj_{\mathbb{R}^{\left[n\right] \choose 2} \times \mathbb{R}^{\left[n\right]^{\underline{3}^{1,2\not>3}}}} P^{K_n, spt, symm, 0} = P^{K_n, spt, asymm, 1}
\end{displaymath}
gilt, ist offensichtlich. Daher werden wir dies nur für $n=3$ beweisen:

Sei (wie schon vorher) $\left(\begin{array}{c}
x \\
z
\end{array}\right)\in P^{K_n, spt, asymm, 1}$
und $\left(\begin{array}{c}
x \\
\widetilde{z}
\end{array}\right)\in P^{K_n, spt, symm, 0}$
mit $\widetilde{z}_{\left(v, w\right), u} = z_{\left(v, w\right), u}$ für $\left(v, w, u\right)\in \left[n\right]^{\underline{3}^{1,2\not>3}}$.

Wegen \eqref{eq:z_n-1_n_n-2} und \eqref{eq:z_n_n-1_n-2} ist
\begin{eqnarray*}
\widetilde{z}_{\left(2, 3\right), 1} & = & 1 - x_{\left\{1, 2\right\}} \\
\widetilde{z}_{\left(3, 2\right), 1} & = & 1 - x_{\left\{1, 3\right\}}.
\end{eqnarray*}

Die Gültigkeit von der fehlenden Gleichungen aus \eqref{spt_erweitert_primal2_2} folgt direkt nach Konstruktion und
\begin{displaymath}
x_{\left\{2, 3\right\}} - \widetilde{z}_{\left(2, 3\right), 1}  - \widetilde{z}_{\left(3, 2\right), 1} = 0
\end{displaymath}
ist eine direkte Konsequenz aus \eqref{eq:z_n-1_n_n-2_plus_z_n_n-1_n-2}.

Somit ist nur noch sicherzustellen, dass 
\begin{eqnarray*}
\widetilde{z}_{\left(2, 3\right), 1} & := & 1 - x_{\left\{1, 2\right\}} \\
& \geq & 0 \\
\widetilde{z}_{\left(3, 2\right), 1} & := & 1 - x_{\left\{1, 3\right\}} \\
...
Da jedoch $P^{K_n, spt, asymm, 1}$ eine erweiterte Formulierung des Spannbaum-Polytops bildet, ist $x\leq 1$, woraus die Aussage direkt folgt.
\end{bew}

\section{Der Zusammenhang zwischen $P^{K_n, spt, symm, 0}$ und $P^{K_n, spt, asymm, 4}$}

Nun wollen anschauen, ob eine analoge Aussage, die Satz \ref{sa:proj_P_p^2_neq_P_p^asymm} für $P^{K_n, spt, asymm, 1}$ trifft, für $P^{K_n, spt, asymm, 4}$ erfüllt ist. Mit anderen Worten: gilt
 \begin{equation}
...
$\widetilde{z}_{\left(v_1, v_2\right), v_3} \geq 0 \ \forall \left(v_1, v_2, v_3\right) \in \left[3\right]^{\underline{3}}$ ist trivialerweise erfüllt -- somit sind nur Gleichungen \eqref{spt_erweitert_primal2_1} und \eqref{spt_erweitert_primal2_2} zu überprüfen.

Gleichung \eqref{spt_erweitert_primal2_2}
\begin{eqnarray*}
...
Somit müssen wir nur absichern, dass Gleichung \eqref{spt_erweitert_primal2_1}
\begin{eqnarray*}
...
\chapter{Die Notwendigkeit der Nichtnegativitätsbedingungen}

In diesem Kapitel soll es um die Frage gehen: sind Nichtnegativitätsbedingungen der erweiterten Formulierungen des Spannbaum-Polytops in dem Sinne irredundant, dass, wenn man sie weglässt, das Ergebnis keine erweiterte Formulierung des Spannbaum-Polytops mehr darstellt?

Für einige der vorgestellten erweiterten Formulierungen wollen wir in diesem Kapitel die Frage beantworten.

\section{Die Nichtnegativitätsbedingungen für $P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}}$}

\begin{Sa}
Wenn man in $P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}}$ nur eine einzige der $z_{\left(v,w\right), u} \geq 0$-Bedingungen weglässt, so ist das resultierende Polyeder keine erweiterte Formulierung des Spannbaum-Poly\-tops mehr.
\end{Sa}
\begin{bew}
Zugunsten einer besseren Lesbarkeit des Beweises (Vermeidung indizierter Indizes) werden wir anstelle des allgemeinen Falls das Resultat nur für die Ungleichung $z_{\left(n, n-1\right), n-2} \geq 0$ zeigen. Der allgemeine Fall folgt aufgrund der Symmetrie von $P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}}$. Offensichtlich ist es nur sinnvoll, $n\geq 3$ zu betrachten, da es für $n\leq 2$ keine $z$-Variablen gibt.

Nehmen wir an, die Ungleichung  $z_{\left(n, n-1\right), n-2} \geq 0$ würde \emph{nicht} gelten.

\paragraph{Fall 1: $n=3$}

Setze
\begin{eqnarray*}
x_{\left\{v, w\right\}} & = & \begin{cases}
2 & \left\{v, w\right\} = \left\{1, 3\right\} \\
0 & \textnormal{sonst}
\end{cases} \\
z_{\left(v, w\right), u} & = & \begin{cases}
2 & \left(v, w, u\right) \in \left\{\left(2, 3, 1\right), \left(3, 1, 2\right)\right\} \\
-2 & \left(v, w, u\right) = \left(3, 2, 1\right) \\
0 & \textnormal{sonst}.
\end{cases}
\end{eqnarray*}

Hier rechnet man einfach nach, dass alle Gleichungen und Ungleichungen erfüllt sind.

\paragraph{Fall 2: $n\geq 4$}

Setze (mit $\left\{v, w\right\} \in {\left[n\right] \choose 2}$ und $\left(v, w, u\right) \in \left[n\right]^{\underline{3}}$)
\begin{eqnarray*}
x_{\left\{v, w\right\}} & = & \begin{cases}
1 & \left\{v, w\right\} = \left\{1, w\right\} \textnormal{ mit } w\notin \left\{n-2, n\right\} \\
2 & \left\{v, w\right\} = \left\{n-2, n\right\} \\
0 & \textnormal{sonst}
\end{cases} \\
z_{\left(v, w\right), u} & = & \begin{cases}
1 & \left(v, w, u\right) \in \left\{\left(1, n-1, n-2\right), \left(1, n-1, n\right)\right\} \\
1 & w=1 \wedge v \leq n-3 \\
1 & \left(v, w\right) = \left(n-2, n\right) \wedge u \neq n-1 \\
1 & \left(v, w\right) = \left(n-1, 1\right) \wedge u \leq n-3 \\
2 & \left(v, w, u\right) = \left(n-1, n, n-2\right) \\
1 & \left(v, w\right) = \left(n, n-2\right), u \neq n-1 \\
2 & \left(v, w, u\right) = \left(n, n-2, n-1\right) \\
-2 & \left(v, w, u\right) = \left(n, n-1, n-2\right) \\
0 & \textnormal{sonst}.
\end{cases}
\end{eqnarray*}

Zu Gleichung \eqref{eq:spt_erweitert_symm_prim_1}:
\begin{displaymath}
x_{\left\{v, w\right\}} - z_{\left(v, w\right), u} - z_{\left(w, v\right), u} = 0\ \forall \left(\left\{v, w\right\}, u\right)\in EV_n.
\end{displaymath}

Sei $\left(\left\{v, w\right\}, u\right)\in EV_n$ und ohne Einschränkung $v<w$.

\subparagraph{Fall 1: $\left(v, w\right) \in \left\{\left(1, n-2\right), \left(1, n\right)\right\}$} Dann gilt $x_{\left\{1, w\right\}} = z_{\left(1, w\right), u} = z_{\left(w, 1\right), u} = 0$.

\subparagraph{Fall 2: $\left(v, w\right) = \left(1, n-1\right)$} Dann ist $x_{\left\{1, n-1\right\}} = 1$.

Falls $u\in \left\{n-2, n\right\}$ ist, ist $z_{\left(1, n-1\right), u} = 1$ und $z_{\left(n-1, 1\right), u} = 0$.

Im Falle von $u\notin \left\{n-2, n\right\}$ ist die Situation umgekehrt: wir haben  $z_{\left(1, n-1\right), u} = 0$ und $z_{\left(n-1, 1\right), u} = 1$.

\subparagraph{Fall 3: $v=1 \wedge w\notin \left\{n-2, n-1, n\right\}$} Es ist $x_{\left\{1, w\right\}} = 1$, $z_{\left(1, w\right), u} = 0$ und $z_{\left(w, 1\right), u} = 1$.

\subparagraph{Fall 4: $\left(v, w\right) = \left(n-2, n-1\right)$} Dann gilt $x_{\left\{n-2, n-1\right\}} = z_{\left(n-2, n-1\right), u} = z_{\left(n-1, n-2\right), u} = 0$.

\subparagraph{Fall 5: $\left(v, w\right) = \left(n-2, n\right)$} Es ist $x_{\left\{n-2, n\right\}} = 2$. Falls $u\neq n-1$ ist, ist $z_{\left(n-2, n\right), u} = z_{\left(n, n-2\right), u} = 1$. Für $u=n-1$ hat man $z_{\left(n-2, n\right), n-1} = 0$ und $z_{\left(n, n-2\right), n-1} = 2$.

\subparagraph{Fall 6: $v=n-1$} Dann ist wegen $v<w$ sicher $w=n$ und es gilt: $x_{\left\{n-1, n\right\}} = 0$.

Falls $u=n-2$ ist, gilt: $z_{\left(n-1, n\right), n-2} = 2$ und $z_{\left(n, n-1\right), n-2} = -2$.

Sonst ist $z_{\left(n-1, n\right), u} = z_{\left(n, n-1\right), u} = 0$.

\subparagraph{Fall 7: $2 \leq v \leq n-3$} Dann haben wir $x_{\left\{v, w\right\}} = 0$ und wegen $v<w$ gilt sicher $w\geq 3$. Dann haben wir $z_{\left(v, w\right), u} = z_{\left(w, v\right), u} = 0$. \\

Zu Gleichung \eqref{eq:spt_erweitert_symm_prim_2}:
\begin{displaymath}
2 x_{\left\{v, w\right\}} + \sum_{u\in[n]\backslash\left\{v, w\right\}} \left(z_{\left(v, u\right), w} + z_{\left(w, u\right), v} \right) = 2\ \forall \left\{v, w\right\}\in {[n] \choose 2}.
\end{displaymath}

Sei $\left\{v, w\right\}\in {[n] \choose 2}$ und wieder ohne Einschränkung $v<w$.

\subparagraph{Fall 1: $\left(v, w\right) \in \left\{\left(1, n-2\right), \left(1, n\right)\right\}$} Dann gilt $x_{\left\{1, w\right\}} = 0$; von den  $z_{\left(1, u\right), w}$ ist genau $z_{\left(1, n-1\right), w} = 1$ und von den $z_{\left(w, u\right), 1}$ ist bei $w=n-2$ genau $z_{\left(n-2, n\right), 1} = 1$ und bei $w=n$ genau $z_{\left(n, n-2\right), 1} = 1$ (alle anderen sind $0$).

\subparagraph{Fall 2: $v=1 \wedge w \notin \left\{n-2, n\right\}$} Dann ist $x_{\left\{1, w\right\}} = 1$ und es gilt für alle $u\neq, 1, w$:  $z_{\left(1, u\right), w} = z_{\left(w, u\right), 1} = 0$.

\subparagraph{Fall 3: $2 \leq v \leq n-3$} Es ist $x_{\left\{v, w\right\}} = 0$. Da nach Voraussetzung $v \leq n-3$ ist, gilt $z_{\left(v, 1\right), w} = 1$ und für $u\neq 1$ ist $z_{\left(v, u\right), w} = 0$.

Falls $w \leq n-3$ ist, ist $z_{\left(w, 1\right), v} = 1$.

Für $w = n-2$ gilt $z_{\left(n-2, n\right), v} = 1$.

Im Falle von $w = n-1$ ist $z_{\left(n-1, 1\right), v} = 1$.

Wenn $w = n$ ist, haben wir $z_{\left(n, n-2\right), v} = 1$.

In jedem Fall gilt für alle sonstigen $u$: $z_{\left(w, u\right), v} = 0$.

\subparagraph{Fall 4: $\left(v, w\right) = \left(n-2, n-1\right)$} Hier ist $x_{\left\{n-2, n-1\right\}} = 0$, $z_{\left(n-1, n\right), n-2} = 2$ und alle sonstigen $z_{\left(n-2, u\right), n-1}$ bzw. $z_{\left(n-1, u\right), n-2}$ sind $0$.

\subparagraph{Fall 5: $\left(v, w\right) = \left(n-2, n\right)$} Hier ist $x_{\left\{n-2, n\right\}} = 2$, $z_{\left(n, n-1\right), n-2} = -2$ und alle sonstigen $z_{\left(n-2, u\right), n}$ bzw. $z_{\left(n, u\right), n-2}$ sind $0$.

\subparagraph{Fall 6: $\left(v, w\right) = \left(n-1, n\right)$} Hier ist $x_{\left\{n-1, n\right\}} = 0$, $z_{\left(n, n-2\right), n-1} = 2$ und alle sonstigen $z_{\left(n-1, u\right), n}$ bzw. $z_{\left(n, u\right), n-1}$ sind $0$.
\end{bew}

\begin{Bem}
Ein weiteres Gegenbeispiel speziell für $n=4$ ist
\begin{eqnarray*}
x_{\left\{v, w\right\}} & = & \begin{cases}
\frac{1}{2} & \left\{v, w\right\} = \left\{1, 3\right\} \\
1 & \left\{v, w\right\} = \left\{1, 4\right\} \\
\frac{3}{2} & \left\{v, w\right\} = \left\{2, 4\right\} \\
0 & \textnormal{sonst}
\end{cases} \\
z_{\left(v, w\right), u} & = & \begin{cases}
\frac{1}{2} & \left(v, w, u\right) \in \left\{\left(1, 3, 2\right), \left(3, 1, 4\right)\right\} \\
1 & \left(v, w, u\right) \in \left\{\left(1, 4, 3\right), \left(4, 1, 2\right)\right\} \\
\frac{3}{2} & \left(v, w, u\right) \in \left\{\left(2, 4, 1\right), \left(4, 2, 3\right)\right\} \\
2 & \left(v, w, u\right) = \left(3, 4, 2\right) \\
-2 & \left(v, w, u\right) = \left(4, 3, 2\right) \\
0 & \textnormal{sonst}
\end{cases}
\end{eqnarray*}
\end{Bem}

\section{Die Nichtnegativitätsbedingungen für $P^{K_n, spt, symm, xy}$}
...
\section{Die Notwendigkeit von $x_{\left\{n-1, n\right\}} \geq 0$ für $P^{K_n, spt, asymm, 1}$}

\begin{Le}
\label{le:xn-1, ngeq0}
Bedingung \eqref{eq:spt_erweitert_asymm_x_n-1n} ($x_{\left\{n-1, n\right\}} \geq 0$) ist in $P^{K_n, spt, asymm, 1}$ für $n\geq 4$ tatsäch\-lich notwendig -- sonst haben wir keine erweiterte Formulierung des Spannbaum-Polytops. Für $n\leq 3$ ist sie in dem Sinne verzichtbar, dass $P^{K_n, spt, asymm, 1}$ beim Weglassen unverändert bleibt.
\end{Le}
\begin{bew}
Dass Bedingung \eqref{eq:spt_erweitert_asymm_x_n-1n} für $n\leq 2$ verzichtbar ist, ist offensichtlich. Daher hier nur Beweis für $n=3$.

Es gilt:
\begin{eqnarray*}
x_{\left\{2, 3\right\}} & \stackrel{\eqref{eq:spt_erweitert_asymm_prim_2}}{=} & 1 - z_{\left(2, 1\right), 3} \\
& \stackrel{\eqref{eq:spt_erweitert_asymm_prim_1}}{=} & 1 + z_{\left(1, 2\right), 3} - x_{\left\{1, 2\right\}} \\
& \stackrel{\eqref{eq:spt_erweitert_asymm_prim_2}}{=} & 1 + z_{\left(1, 2\right), 3} + z_{\left(1, 3\right), 2} -1 \\
& = & z_{\left(1, 2\right), 3} + z_{\left(1, 3\right), 2} \\
& \stackrel{z\geq 0}{\geq} & 0.
\end{eqnarray*}

Nun zu \eqref{eq:spt_erweitert_asymm_x_n-1n} ist unverzichtbar für $n\geq 4$:

Setze für $\left\{v, w\right\} \in {[n]\choose 2}$ mit $v<w$:
\begin{displaymath}
x_{\left\{v, w\right\}} := 
\begin{cases}
- (n-3) & \left\{v, w\right\} = \left\{n-1, n\right\} \\
1 & v < n-1 \wedge w = n-1 \\
1 & v < n-1 \wedge w = n \\
0 & \textnormal{sonst}
\end{cases}
\end{displaymath}
und für $\left(v, w, u\right) \in \left[n\right]^{\underline{3}^{1,2\not>3}}$:
\begin{displaymath}
z_{\left(v, w\right), u} := 
\begin{cases}
1 & v < n-1 \wedge w = n \wedge u < n-1\\
1 & v = n-1 \\
1 & v = n \wedge u=n-1 \\
0 & \textnormal{sonst}
\end{cases}
\end{displaymath}
Wir überprüfen die Gleichungen \eqref{eq:spt_erweitert_asymm_prim_1} und \eqref{eq:spt_erweitert_asymm_prim_2}:

Zu \eqref{eq:spt_erweitert_asymm_prim_1}:

Sei $\left(\left\{v, w\right\}, u\right)\in EV_n^{\not>}$ und es gelte ohne Einschränkung $v<w$.

Als erstes möchten wir anmerken, dass hieraus $v < n-1$ folgt: wegen $v<w$ muss sicher $v<n$ sein. Falls $v = n-1$ wäre, so wäre $w=n$ und es gäbe kein $u\in \left[n\right]$ mit $\left(\left\{v, w\right\}, u\right)\in EV_n^{\not>}$.

\paragraph{Fall 1: $w < n-1$} 
Dann ist $x_{\left\{v, w\right\}} = 0$, $z_{\left(v, w\right), u} = 0$,  $z_{\left(w, v\right), u} = 0$ und es gilt sicherlich
\begin{displaymath}
x_{\left\{v, w\right\}} - z_{\left(v, w\right), u} - z_{\left(w, v\right), u} = 0.
\end{displaymath}

\paragraph{Fall 2: $w=n-1$}
Hier haben wir $x_{\left\{v, n-1\right\}} = 1$, $z_{\left(v, n-1\right), u} = 0$ und $z_{\left(n-1, v\right), u} = 1$ und es gilt
\begin{displaymath}
x_{\left\{v, n-1\right\}} - z_{\left(v, n-1\right), u} - z_{\left(n-1, v\right), u} = 0.
\end{displaymath}

\paragraph{Fall 3: $w = n \wedge u < n-1$}
Dann gilt $x_{\left\{v, n\right\}} = 1$, $z_{\left(v, n\right), u} = 1$ und $z_{\left(n, v\right), u} = 0$ und damit
\begin{displaymath}
x_{\left\{v, n\right\}} - z_{\left(v, n\right), u} - z_{\left(n, v\right), u} = 0.
\end{displaymath}

\paragraph{Fall 4: $w = n \wedge u = n-1$}
Hier ist $x_{\left\{v, n\right\}} = 1$, $z_{\left(v, n\right), n-1} = 0$ und $z_{\left(n, v\right), n-1} = 1$ und es gilt
\begin{displaymath}
x_{\left\{v, n\right\}} - z_{\left(v, n\right), n-1} - z_{\left(n, v\right), n-1} = 0.
\end{displaymath}

Zu \eqref{eq:spt_erweitert_asymm_prim_2}:

Sei $\left(v, w\right)\in {[n]^{\underline{2}}}^<$.

\paragraph{Fall 1: $v < n-1 \wedge w < n-1$}
In diesem Fall ist $x_{\left\{v, w\right\}} = 0$. Für genau ein $u\in[n]\backslash\left\{v, w\right\}$ (nämlich $u=n$) gilt: $z_{\left(v, u\right), w} = 1$. Hieraus folgt
\begin{displaymath}
x_{\left\{v, w\right\}} + \sum_{u\in[n]\backslash\left\{v, w\right\}} z_{\left(v, u\right), w} = 1.
\end{displaymath}

\paragraph{Fall 2: $v < n-1 \wedge w = n-1$}
Dann ist $x_{\left\{v, n-1\right\}} = 1$ und es gilt für alle $u\in[n]\backslash\left\{v, n-1\right\}$: $z_{\left(v, u\right), n-1} = 0$, woraus
\begin{displaymath}
x_{\left\{v, n-1\right\}} + \sum_{u\in[n]\backslash\left\{v, n-1\right\}} z_{\left(v, u\right), n-1} = 1
\end{displaymath}
folgt.

\paragraph{Fall 3: $v < n-1 \wedge w = n$}
Hier haben wir $x_{\left\{v, n\right\}} = 1$ und es gilt für alle $u\in[n]\backslash\left\{v, n\right\}$: $z_{\left(v, u\right), n} = 0$, also
\begin{displaymath}
x_{\left\{v, n\right\}} + \sum_{u\in[n]\backslash\left\{v, n\right\}} z_{\left(v, u\right), n} = 1.
\end{displaymath}

\paragraph{Fall 4: $\left(v, w\right) = \left(n-1, n\right)$}
Dann ist $x_{\left\{n-1, n\right\}} = -\left(n-3\right)$ und für alle $u\in[n]\backslash\left\{n-1, n\right\}$ gilt: $z_{\left(n-1, u\right), n} = 1$. Dann ist sicher
\begin{displaymath}
x_{\left\{n-1, n\right\}} + \sum_{u\in[n]\backslash\left\{n-1, n\right\}} z_{\left(n-1, u\right), n} = 1
\end{displaymath}
erfüllt.
\end{bew}
...
\end{Sa}
\begin{bew}
Aus Gleichung \eqref{spt_erweitert_primal2_1} folgt mit $z_{\left(v, w\right), u} \geq 0 \ \forall \left(v, w, u\right) \in \left[n\right]^{\underline{3}}$ für ein beliebiges $u \in \left[n\right] \backslash \left\{v, w\right\}$:
\begin{displaymath}
...
Mit Gleichung \eqref{spt_erweitert_primal2_2} folgt:
\begin{eqnarray*}
...
Betrachte Gleichung \eqref{spt_erweitert_primal2_2}
\begin{eqnarray*}
1 & = & x_{\left\{v, \widetilde{u}\right\}} + \sum_{u\in[n]\backslash\left\{v, \widetilde{u}\right\}} z_{\left(v, u\right), \widetilde{u}} \\
...
Es folgt aus Gleichung \eqref{spt_erweitert_primal2_2}:
\begin{eqnarray}
...
Es gilt nach \eqref{spt_erweitert_primal2_1} und wegen $w_j \prec^+ v$:
\begin{eqnarray}
...
Aus Gleichung \eqref{spt_erweitert_primal2_1} folgt unter Beachtung von $w_j \prec^+ v$:
\begin{eqnarray}
z_{\left(v, w_j\right), \widetilde{u}} & = & x_{\left\{v, w_j\right\}} - z_{\left(w_j, v\right), \widetilde{u}} \nonumber \\
...
\section{Die Einbettung der aufspannenden Bäume in $P^{K_n, spt, asymm, 1}$}

Nun kann man sich die Frage stellen, inwieweit die Einbettung der aufspannenden Bäume in $P^{K_n, spt, asymm, 1}$ eindeutig ist. Diese Frage lässt sich negativ beantworten.

Betrachte dazu den Vektor für $n=4$ aus dem Beweis von Lemma \ref{le:keine_proj_P_p^2_P_p^asymm_1}, für $n=5$ das Beispiel aus Bemerkung \ref{bem:keine_proj_P_p^2_P_p^asymm_1}, für alle $n\geq 4$ die Vektoren aus dem Beweis von Lemma \ref{le:keine_proj_P_p^2_P_p^asymm_2}, sowie für $n=5$ auch Bemerkung \ref{bem:keine_proj_P_p^2_P_p^asymm_2}.

An dieser Stelle wollen wir lediglich die beiden Beispiele für $n=4$ nochmals wiedergeben:

\begin{Bsp}
Sei $T=\left\{\left\{1, 3\right\}, \left\{1, 4\right\}, \left\{2, 3\right\}\right\}$ und $x=\chi\left(T\right)$. Dann existiert neben dem "`kanonischen"' $z$
\begin{displaymath}
z_{\left(v, w\right), u} = \begin{cases}
1 & \left(v, w, u\right) \in \left\{\left(1, 3, 2\right), \left(2, 3, 1\right), \left(2, 3, 4\right), \left(3, 1, 4\right), \left(4, 1, 2\right), \left(4, 1, 3\right)\right\} \\
0 & \textnormal{sonst}
\end{cases}
\end{displaymath}
auch noch
...
\end{cases}
\end{displaymath}
so dass $\left(\begin{array}{c}
x \\
z\end{array}\right) \in P^{K_n, spt, asymm, 1}$ ist.

Mittels Konvexkombinationen lässt sich eine durch $\lambda \in \left[0, 1\right]$ indizierte Familie von $z$
\begin{displaymath}
z_{\left(v, w\right), u} = \begin{cases}
1 & \left(v, w, u\right) \in \left\{ \left(2, 3, 4\right), \left(3, 1, 4\right), \left(4, 1, 3\right) \right\} \\
\lambda & \left(v, w, u\right) \in \left\{ \left(1, 3, 2\right), \left(2, 3, 1\right), \left(4, 1, 2\right) \right\} \\
1-\lambda & \left(v, w, u\right) \in \left\{\left(1, 4, 2\right), \left(3, 1, 2\right) \right\} \\
0 & \textnormal{sonst}
\end{cases}
\end{displaymath}
mit $\left(\begin{array}{c}
x \\
z\end{array}\right) \in P^{K_n, spt, asymm, 1}$ bauen.
\end{Bsp}

\begin{Bsp}
Sei $T=\left\{\left\{1, 3\right\}, \left\{1, 4\right\}, \left\{2, 4\right\}\right\}$ und $x=\chi\left(T\right)$. Dann existiert neben dem "`kanonischen"' $z$
\begin{displaymath}
z_{\left(v, w\right), u} = \begin{cases}
1 & \left(v, w, u\right) \in  \left\{\left(1, 4, 2\right), \left(2, 4, 1\right), \left(2, 4, 3\right), \left(3, 1, 2\right), \left(3, 1, 4\right), \left(4, 1, 3\right)\right\} \\
0 & \textnormal{sonst}
\end{cases}
\end{displaymath}
auch noch folgendes
...
so dass $\left(\begin{array}{c}
x \\
z\end{array}\right) \in P^{K_4, spt, asymm, 1}$ ist.

Mittels Konvexkombinationen lässt sich eine durch $\lambda \in \left[0, 1\right]$ indizierte Familie von $z$
\begin{displaymath}
z_{\left(v, w\right), u} = \begin{cases}
1 & \left(v, w, u\right) \in \left\{ \left(2, 4, 3\right), \left(3, 1, 4\right), \left(4, 1, 3\right) \right\} \\
\lambda & \left(v, w, u\right) \in \left\{ \left(1, 4, 2\right), \left(2, 4, 1\right), \left(3, 1, 2\right) \right\} \\
1-\lambda & \left(v, w, u\right) \in \left\{\left(1, 3, 2\right), \left(4, 1, 2\right) \right\} \\
0 & \textnormal{sonst}
\end{cases}
\end{displaymath}
mit $\left(\begin{array}{c}
...
\subsection{Untersuchung von $P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}}$}

Es sei daran erinnert (vgl. Satz \ref{sa:spt_erw_symmetrisch}), dass
\begin{eqnarray*}
P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}} & := & \left\{
\left(\begin{array}{c}
x \\
...
Somit ist das homogene lineare Gleichungssystem
\begin{eqnarray*}
\sum_{u\in[n]\backslash\left\{v, w\right\}} q_{\left\{v, w\right\}, u} + 2 r_{\left\{v, w\right\}} & = & 0 \ \forall \left\{v, w\right\}\in {[n] \choose 2}\ \left(x_{\left\{v, w\right\}}\right) \\
-q_{\left\{v, w\right\}, u} + r_{\left\{v, u\right\}} & = & 0\ \forall \left(v, w, u\right)\in [n]^{\underline{3}}\ \left(z_{\left(v, w\right), u}\right)
\end{eqnarray*}
zu analysieren.

\begin{Le}
\label{le:kern_symm_vereinfacht}
Betrachte das homogene lineare Gleichungssystem
\begin{eqnarray}
-q_{\left\{v, w\right\}, u} + r_{\left\{v, u\right\}} & = & 0\ \forall \left(v, w, u\right)\in [n]^{\underline{3}}  \label{eq:le_kern_symm_vereinfacht}
\end{eqnarray}
mit
\begin{displaymath}
\left(\begin{array}{c}
...
Sei $\lambda\in\mathbb{R}$ vorgegeben. Setze
\begin{eqnarray*}
r_{\left\{i, j\right\}} & := & \lambda\ \forall \left\{i, j\right\}\in\left[n\right]^2 \\
q_{\left\{i, j\right\}, k} & := & \lambda\ \forall \left(\left\{i, j\right\}, k\right)\in EV_n.
\end{eqnarray*}
\end{Le}
\begin{bew}
Zuerst zeigen wir, dass $r_{\left\{v, w\right\}} = r_{\left\{v', w'\right\}}$ für $\left\{v, w\right\} \neq \left\{v', w'\right\}$ ist.

Sei ohne Einschränkung $w\neq w'$.

\begin{eqnarray*}
r_{\left\{v, w\right\}} & = & q_{\left\{v, w'\right\}, w} \\
& = & r_{\left\{w, w'\right\}} \\
& = & q_{\left\{v', w\right\}, w'} \\
& = & r_{\left\{v', w'\right\}}
\end{eqnarray*}

Somit gilt (unter Beachtung von $-q_{\left\{v, w\right\}, u} + r_{\left\{v, u\right\}} = 0$) die Aussage.
\end{bew}

\begin{Kor}
\label{kor:linearisiert_C_symm_vereinfacht}
Betrachte das lineare Gleichungssystem
\begin{eqnarray}
\sum_{u\in[n]\backslash\left\{v, w\right\}} q_{\left\{v, w\right\}, u} + 2 r_{\left\{v, w\right\}} & = & 0 \ \forall \left\{v, w\right\}\in {[n] \choose 2} \label{kor:kern1_symm_vereinfacht} \\
-q_{\left\{v, w\right\}, u} + r_{\left\{v, u\right\}} & = & 0\ \forall \left(v, w, u\right)\in [n]^{\underline{3}} \nonumber
\end{eqnarray}
mit
\begin{displaymath}
\left(\begin{array}{c}
...
Dieses lineare Gleichungssystem hat als Lösung genau die Nulllösung.
\end{Kor}
\begin{bew}
Die Eindeutigkeit folgt, indem wir die Lösung aus Lemma \ref{le:kern_symm_vereinfacht} in Gleichung \eqref{kor:kern1_symm_vereinfacht} einsetzen.
\end{bew}
...
\subsection{Untersuchung von $P^{K_n, spt, asymm, 1}$}

$P^{K_n, spt, asymm, 1}$ ist nach Satz \ref{sa:spt_erw_asymmetrisch} durch folgende Gleichungen und Ungleichungen beschrieben:
\begin{eqnarray*}
P^{K_n, spt, asymm, 1} & = & \left\{
\left(\begin{array}{c}
x \\
z
\end{array}\right)\in \mathbb{R}^{[n]\choose 2}\times \mathbb{R}^{\left[n\right]^{\underline{3}^{1,2\not>3}}}: \nonumber\right. \\
 x_{\left\{v, w\right\}} - z_{\left(v, w\right), u} - z_{\left(w, v\right), u} & = & 0 \ \forall \left(\left\{v, w\right\}, u\right)\in EV_n^{\not>}\  \left(q_{\left\{v, w\right\}, u}\right) \\
x_{\left\{v, w\right\}} + \sum_{u\in[n]\backslash\left\{v, w\right\}} z_{\left(v, u\right), w} & = & 1\ \forall \left(v, w\right)\in {[n]^{\underline{2}}}^< \left(r_{\left(v, w\right)}\right) \\
x_{\left\{n-1, n\right\}} & \geq & 0 \\
z_{\left(v, w\right), u} & \geq & \left. 0 \ \forall \left(v, w, u\right) \in \left[n\right]^{\underline{3}^{1,2\not>3}} \right\}.
\end{eqnarray*}

Somit ist folgendes homogenes lineare Gleichungssystem zu betrachten:
\begin{eqnarray*}
\sum_{u=v+1, u\neq w}^n q_{\left\{v, w\right\}, u} + r_{\left(v, w\right)} & = & 0\ \forall \left(v, w\right)\in {[n]^{\underline{2}}}^<\ \left(x_{\left\{v, w\right\}}\right) \\
-q_{\left\{v, w\right\}, u} + r_{\left(v, u\right)} & = & 0\ \forall \left(v, w, u\right)\in [n]^{\underline{3}^{1<3}} \ \left(z_{\left(v, w\right), u}\right) \\
-q_{\left\{v, w\right\}, u} & = & 0\ \forall \left(v, w, u\right)\in \left[n\right]^{\underline{3}^{2<3<1}} \ \left(z_{\left(v, w\right), u}\right).
\end{eqnarray*}

\begin{Le}
\label{le:lsg_P_asymm1_part}
Betrachte das homogene lineare Gleichungssystem
\begin{eqnarray}
-q_{\left\{v, w\right\}, u} + r_{\left(v, u\right)} & = & 0\ \forall \left(v, w, u\right)\in [n]^{\underline{3}^{1<3}} \ \left(z_{\left(v, w\right), u}\right) \label{eq:asymm_qr2_1} \\
-q_{\left\{v, w\right\}, u} & = & 0\ \forall \left(v, w, u\right)\in \left[n\right]^{\underline{3}^{2<3<1}} \ \left(z_{\left(v, w\right), u}\right) \label{eq:asymm_qr2_2}
\end{eqnarray}
mit
\begin{displaymath}
\left(\begin{array}{c}
...
r
\end{array}\right) \in 
\mathbb{R}^{EV_n^{\not>}}\times {\mathbb{R}^{\underline{2}}}^<.
\end{displaymath}

Jede Lösung dieses linearen Gleichungssystems lässt sich dann folgendermaßen darstellen:

Sei $\lambda\in\mathbb{R}$ vorgegeben.
Setze
\begin{eqnarray*}
r_{\left(i, j\right)} & := & \begin{cases}
0 & j<n \\
\lambda & j=n
\end{cases}\\
q_{\left\{i, j\right\},k} & := & \begin{cases}
0 & k<n \\
\lambda & k=n.
\end{cases}
\end{eqnarray*}
\end{Le}
\begin{bew}
Sei $u\neq n$ und $v<u$. Dann ist (Gleichung \eqref{eq:asymm_qr2_2})
\begin{displaymath}
-q_{\left\{n, v\right\}, u} = 0.
\end{displaymath}

Gleichzeitig gilt (Gleichung \eqref{eq:asymm_qr2_1}):
\begin{displaymath}
-q_{\left\{v, n\right\}, u} + r_{\left(v, u\right)} = 0,
\end{displaymath}
also
\begin{displaymath}
r_{\left(v, u\right)} = 0.
...
Daraus folgt (Gleichung \eqref{eq:asymm_qr2_1}), dass für $u < n$ (woraus $v<u$ oder $w<u$ nach Konstruktion von $EV_n^{\not>}$ folgt) gilt:
\begin{displaymath}
q_{\left\{v, w\right\}, u} = 0.
\end{displaymath}

Nun zum Fall $u=n$. Es gilt für $v\neq v'$ mit $v, v' < n$ (Gleichung \eqref{eq:asymm_qr2_1}):
\begin{displaymath}
-q_{\left\{v, v'\right\}, n} + r_{\left(v, n\right)} = 0,
\end{displaymath}
aber auch
\begin{displaymath}
-q_{\left\{v, v'\right\}, n} + r_{\left(v', n\right)} = 0,
...
Somit ist
\begin{eqnarray*}
r_{\left(v, n\right)} & = & r_{\left(v', n\right)} \\
r_{\left(v, n\right)} & = & q_{\left\{v, v'\right\}, n}.
\end{eqnarray*}

Nachrechnen ergibt, dass diese Variablenbelegungen tatsächlich Lösungen sind.
\end{bew}

\begin{Kor}
\label{kor:linearisiert_C_asymm_vereinfacht}
Betrachte das homogene lineare Gleichungssystem
\begin{eqnarray*}
\sum_{u=v+1, u\neq w}^n q_{\left\{v, w\right\}, u} + r_{\left(v, w\right)} & = & 0\ \forall \left(v, w\right)\in {[n]^{\underline{2}}}^<\ \left(x_{\left\{v, w\right\}}\right) \\
-q_{\left\{v, w\right\}, u} + r_{\left(v, u\right)} & = & 0\ \forall \left(v, w, u\right)\in [n]^{\underline{3}^{1<3}} \ \left(z_{\left(v, w\right), u}\right) \\
...
Dieses homogene lineare Gleichungssystem hat ausschließlich die Nulllösung.
\end{Kor}
\begin{bew}
Einsetzen der Lösung aus Lemma \ref{le:lsg_P_asymm1_part}.
\end{bew}
...
\begin{eqnarray*}
\dim P^{K_n, spt, symm, 0} & = & \frac{\left(n-1\right)^2 \left(n-2\right)}{2} \\
\dim P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}} & = & \frac{n \left(n-1\right)\left(n-2\right)}{2} \\
\dim P^{K_n, spt, symm, xy} & = & \frac{n \left(n-1\right)\left(n-2\right)}{3} \\
\dim P^{K_n, spt, asymm, 1} & = & \frac{n \left(n-1\right)\left(n-2\right)}{3} \\
...
\begin{itemize}
\item $n \left(n-1\right) \left(n-2\right)$ für $P^{K_n, spt, symm, 0}$ (bezüglich Beschreibung $\mathcal{I}^{n, symm, 0}_2$; vgl. Bemerkung \ref{bem:spt_erweitert_dual2}) und  $P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}}$ (vg. Bemerkung \ref{bem:spt_erw_symmetrisch})
\item $\frac{1}{2} n \left(n-1\right) \left(n-2\right)$ für $P^{K_n, spt, symm, xy}$ (vgl. Bemerkung \ref{bem:gleichungen_Pxy})
\item $\frac{2}{3} n \left(n-1\right) \left(n-2\right)$ für $P^{K_n, spt, asymm, 1}$ (vgl. Bemerkung \ref{bem:gleichungen_asymm})
...
\item $\frac{1}{2} n \left(n-1\right) \left(n-2\right) + \frac{1}{2} n \left(n-1\right)$ für $P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}}$ (vg. Bemerkung \ref{bem:spt_erw_symmetrisch})
...
\item $\frac{1}{3} n \left(n-1\right) \left(n-2\right) + \frac{1}{2} n \left(n-1\right)$ für $P^{K_n, spt, asymm, 1}$ (vgl. Bemerkung \ref{bem:gleichungen_asymm})
...
\item $0$ für $P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}}$ (vgl.  Korollar \ref{kor:linearisiert_C_symm_vereinfacht}), $P^{K_n, spt, symm, xy}$ (vgl. Korollar \ref{kor:loesung_transponiert_nsymmxy}), $P^{K_n, spt, asymm, 1}$ (vgl. Korollar \ref{kor:linearisiert_C_asymm_vereinfacht}) und $P^{K_n, spt, asymm, 4}$ (vgl. Lemma \ref{le:linearisiert_asymm4_vereinfacht})
\end{itemize}
...
Zu $P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}}$:
\begin{eqnarray*}
\dim P^{K_n, spt, symm, r_{\left(v, w\right)}+r_{\left(w, v\right)}} & = & \frac{n \left(n-1\right)}{2}+n \left(n-1\right)(n-2)  -\left(\frac{n \left(n-1\right)\left(n-2\right)}{2}+\frac{n \left(n-1\right)}{2}\right) \\
& = & \frac{n \left(n-1\right)\left(n-2\right)}{2}.
\end{eqnarray*}
...
Für $P^{K_n, spt, asymm, 1}$ erhält man
\begin{eqnarray*}
\dim P^{K_n, spt, asymm, 1} & = & \frac{n \left(n-1\right)}{2}+\frac{2}{3} n \left(n-1\right)(n-2)  -\left(\frac{n \left(n-1\right)\left(n-2\right)}{3}+\frac{n \left(n-1\right)}{2}\right) \\
& = & \frac{n \left(n-1\right)\left(n-2\right)}{3}.
\end{eqnarray*}
